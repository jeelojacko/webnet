...
Generated main questions and rubric prompts record `questionTier`: Tier A is a high-confidence semantic actor/action/object prompt, Tier B is source-heading or subsection anchored, and Tier C is a safe heading fallback. Specialized main-question topics such as offences, notice, filing, appeal, certification, application, or regulation-making require positive heading or operative-text evidence before the template is used; otherwise generation falls back to the heading-anchored Tier C question. The audit also emits `MAIN_QUESTION_UNSUPPORTED_TOPIC` if a specialized question survives without source support.

The audit includes non-persisted `suggestedChunks` for sections that are too large for one learnable unit, including many-subsection sections, long enumerations, definition sets, and sections with multiple independent procedural topics. These suggestions are advisory only; the browser UI still creates one study unit unless a later UI batch adds the suggested-unit preview flow.
...
Creating a source-linked legal unit now generates structured rubric items in addition to the older concept suggestions. For legal provisions, `generateStudyRubric` classifies the section topic from the section heading first, then extracts deterministic legal facts from subsection text using statutory signals such as `subject to`, `if`, `on receiving`, `may`, `shall`, `shall not`, `commits an offence`, `punishable`, `notice`, numeric deadlines, filing/registry language, legal-effect wording, and cross-references. Legal text is prepared into exact source text, operative text, amendment-history text, and consolidation-note text before fact extraction or answer generation, so generator code uses operative text by default without partial citation residue. Related facts such as offence plus penalty, duty plus filing consequence, and permission plus trigger are merged before prompt generation so generated prompts test a specific legal subject instead of repeating a generic section-level stem. Generated reference answers stay close to operative source wording, remove amendment-history and consolidation-note text by default, and keep survey-relevance notes explicitly labelled as study notes.

Phase 2E.3 adds semantic regression locks for Surveys Act sections 2-6, Boundaries Confirmation Act section 14, Community Planning Act section 16, Registry Act section 16, and preserves the existing high-quality Community Planning Act section 83 and Boundaries Confirmation Act section 16 outputs. These golden sections prefer broad, accurate questions over unsupported specialized templates and include natural actor/action rubric prompts where the source text is clear.
