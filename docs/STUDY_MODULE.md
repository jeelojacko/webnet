...
- **Safe new-tab links**: `studyWindow.ts` centralizes `noopener,noreferrer` new-tab opening and the popup-block `null` fallback. Active-session source links (Locate reveal exact provision, Drill reveal required lookups, Mock grading/results expected blocks, Locate/Mock Open Statute Library) open in a NEW tab through `EXAM_PREP_OPEN_SOURCE_BUTTON newTab` / `openStudyUrlNewTab`, so an active Locate/Drill/Mock never loses component-local state; Learn list chips keep in-SPA navigation.
- **Ephemeral Locate picker (browser-only)**: `examPrepLocatePicker.ts` defines the `?studyPicker=locate&prompt=…&token=…` protocol. The sprint tab opens the Library in a new tab; picker-mode Library and document-reader pages render the frozen prompt only and post same-origin `study-locate-pick` messages: search results carry `[ Use this document ]` (posts `documentId` with `sourceKey: null`) and `[ Use this provision ]` (posts the exact `sourceKey`) action buttons, opened documents offer `Select this provision` on components and expanded subsections, and every picker-mode in-tab navigation preserves the picker query. The sprint token-guards and objectively scores picks (`documentId` + exact pinned `sourceKey`; document-level targets accept any provision in the right statute), persists the immutable attempt through the ordinary path, and advances the frozen sprint. Nothing is persisted, the expected location never appears in the picker tab, stale tokens/duplicate deliveries are ignored, and Check Answer + self-report remain the fallback.
...
- **Recognition wording**: `EXAM_PREP_RECOGNITION_ASK` (“Which law or legal topic applies?”) and `EXAM_PREP_RECOGNITION_PRACTICE_COPY` are shared by the Recognition sprint, Mock prompt/grading, and Home flow copy (frozen cues/scoring unchanged).
...
- **QA reports**: `npm run study:exam-prep:qa-report` builds byte-deterministic `reports/exam-prep/recall-quality-audit.md` (all 57 cards) and `reports/exam-prep/recognition-quality-audit.md` (all 317 cues) from the frozen pools with mechanical flags and an A-SURV-03 human-review note (three fragmented recall entries documented verbatim — never rewritten). Pure builder: `src/study/examPrep/qa/examPrepQaAudit.ts`.
