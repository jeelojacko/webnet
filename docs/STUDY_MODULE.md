...
- The sidebar item is named **Exam Prep** and routes to `/study/exam-prep`; tabs are Home, Learn (`/study/learn`), Recall (`/study/review`), Recognition (`/study/recognition`), Locate (`/study/locate`), and Lookup Drills (`/study/drills`). The legacy `/study/exam-curriculum` path renders Learn, preserving old bookmarks.
