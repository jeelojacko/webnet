...
Its source of truth is `study-desktop/src/` (standalone Tauri 2 app foundation); the host browser `/study` route is a transitional shim (`src/study/StudyApp.tsx`) re-exporting the relocated `StudyApp`. The standalone shell, Vite/TS boundary, and OPFS helpers live under `study-desktop/` (see `study-desktop/README.md`).

It does not use adjustment, parser, solver, network, or Survey CAD domain state.

## File Structure

- `study-desktop/src/studyTypes.ts` - record contracts for documents, units, prompts, concepts, rubrics, progress, attempts, drafts, settings, and snapshots
- `study-desktop/src/studySeed.ts` - five initial New Brunswick statute document records plus sample units, concepts, prompts, and initial progress
- `study-desktop/src/studyScheduler.ts` - StudyPhase transition and initial-progress rules
- `study-desktop/src/studyQueue.ts` - pure Phase 3 FSRS-aware queue construction with explicit queue reasons
- `study-desktop/src/studyStorage.ts` - IndexedDB schema, seed loading, migrations, CRUD/replace operations, on-demand legal-component repository lookups, atomic rating/undo writes, and atomic official-content package import
- `study-desktop/src/studyStorageTypes.ts` - public Study storage interface shared by persistence helpers
- `study-desktop/src/studyOfficialContent.ts` - official package parsing, browser import validation, preview, source-reference review flags, reference-only form detection, and source-selection study-unit creation
- `study-desktop/src/studyDraftGeneration.ts` - deterministic title, question, citation, reference-answer, and conservative concept drafting for source-linked study units
- `study-desktop/src/studyRubricGeneration.ts` - deterministic answer-rubric templates and legal-provision rubric generation
- `study-desktop/src/studyQuestionSupport.ts` - shared main-question evidence gating, Tier A/B/C metadata, unsupported-topic checks, and suggested study chunk rules
- `study-desktop/src/fsrs/*` - Phase 3 FSRS adapter, Study-domain scheduler settings, JSON-safe card/review-log serialization, parameter validation, and fixed/system clocks
- `study-desktop/src/search/*` - derived MiniSearch record builders, IndexedDB search-index persistence, worker protocol, worker implementation, and service wrapper for Library search
- `study-desktop/src/ai/*` - provider-neutral AI authoring schemas, Study Map result validation/reconciliation, grounding checks, and approval mapping into normal Study records
- `study-desktop/src/studyLibrarySearch.ts` - legacy highlighting helpers and compatibility tests for the former in-memory search path
- `study-desktop/src/studyOpfs.ts` - OPFS path generation and source-file text asset writes
- `study-desktop/src/studyExportImport.ts` - JSON export/import round trip helpers
- `study-desktop/src/studyReviewTransaction.ts` - FSRS rating preview, counted/non-counted attempt assembly, StudyPhase preservation, and latest-rating undo state restoration
- `study-desktop/src/useStudyApp.ts` - route-local state, autosave, reveal/rating flow, latest-rating undo, and persistence orchestration
- `study-desktop/src/useStudyExamPrep.ts` - Exam Prep persistence actions (studied toggles, atomic stale-safe recall ratings, session settings) wired into `useStudyApp`
- `study-desktop/src/examPrep/*` - Exam Prep namespace: pure recall-task derivation from the frozen curriculum manifest (tasks carry `reviewWeight` + `curriculumIndex`), current-binding selectors + recommended-unit picker, shared rank-priority helpers, deterministic recall queue, FSRS rating preview/attempt builders, session-settings helpers, and the tabbed Home / Learn / Recall / Lookup Drills page (Recall = explicit frozen sessions); Phase 2.5 adds the pure `examPrepToday.ts` (local-date activity), `examPrepRecommendations.ts` (Recommended Now + recommended drill), and `examPrepDrillFilters.ts` (drill rows/status/difficulty filters) helpers; Phase 3 adds the `mock/` namespace (`examPrepMockProfiles`, `examPrepMockPaper`, `examPrepMockSession`, `examPrepMockResults`, `examPrepMockSelectors`, `examPrepMockTypes`) plus the focused Mock Exam components (`ExamPrepMockExam`, `ExamPrepMockActive`, `ExamPrepMockGrading`, `ExamPrepMockResultsView`)
- `study-desktop/src/StudyApp.tsx` and `study-desktop/src/components/*` - dashboard, library, document, session, and manage pages
- `study-desktop/tests/*` - focused storage, scheduler, FSRS, queue, OPFS, separation, autosave, UI, and attempt-persistence coverage

## IndexedDB Schema (browser `/study`)

Browser database: `webnet.study.v1` (the Tauri desktop runtime uses native SQLite schema v1 instead; see Native Desktop Persistence below)
...
- `examPrepMockSessions`, key `id`: one record per mock exam session (`${curriculumContentHash}::mock-…`), holding the profile snapshot used at start, the seed, question refs (q01..), free-text responses, self-grading, and status (`in_progress` / `submitted` / `graded` / `abandoned`); indexes `byCurriculumId`, `byCurriculumContentHash`, `byStatus`, `byStartedAt`, `byProfileId`
...
Schema migration currently normalizes imported or partially missing snapshots to schema version `10`, fills default settings, defaults official-content stores to empty arrays, adds unit source mode, adds concept origin/order fields, adds the rubric store, adds Study FSRS settings/configuration, adds optional progress/attempt scheduling wrappers, creates native lookup indexes, repairs malformed legacy `legalComponents` key paths, and keeps existing Study data intact. Existing concepts without origin default to manual unless the linked unit already records generated concepts. Legacy snapshots without rubrics receive conservative supplemental `custom` rubric rows from existing concepts with empty reference answers, so migration does not invent legal answers or delete concept content. Old-schema IndexedDB fixture coverage locks browser upgrade behavior for pre-FSRS and v5/v6 data: missing stores are created, historical attempts are preserved, malformed official legal component stores are recreated for reimport, legal lookup indexes exist, and ambiguous legacy due dates become uninitialized FSRS schedules instead of replayed memory history.

Version `7` specifically repairs browsers that created `legalComponents` with an old plain `id` key path. Full-corpus packages reuse component ids such as `section:1` across documents, so that malformed store can collapse thousands of components to the number of unique local ids. The repair drops only that authoritative imported legal-text store and recreates it with `recordKey`; reimporting the official package restores the complete component set without touching Study units, progress, attempts, drafts, or settings. Version `8` adds AI authoring proposal stores without converting proposals into StudyUnits. Version `9` adds the four Exam Prep stores (`examPrepUnitProgress`, `examPrepRecallProgress`, `examPrepAttempts`, `examPrepSettings`) with curriculum/task indexes (`byCurriculumId`, `byCurriculumContentHash`, `byTaskId`, `byCurriculumAndTask` on recall/attempt stores, `byUnitId` on unit progress, `byKind` on attempts, `byCurriculumId` on settings) without touching existing Study, legal, AI, settings, or FSRS data; v8 snapshots and databases migrate with empty Exam Prep arrays/stores. Version `10` is the deliberate Exam Prep mock phase: it adds exactly one new store, `examPrepMockSessions` (mock sessions keep their own evidence history so a 150-minute mock survives reload/crash/restart), leaving the previous four Exam Prep stores untouched; v9 databases and snapshots migrate additively with `examPrepMockSessions` defaulting to `[]` and no fabricated mock history.

## Native Desktop Persistence (Phase 2, Tauri shell)

Browser `/study` uses IndexedDB `webnet.study.v1` v10 plus OPFS `study/documents/...` assets. The Tauri desktop runtime uses Rust rusqlite native schema v1 at app-data `webnet-study/study.sqlite3` (one `(key, payload-JSON)` table per browser logical store, including opaque derived search artifacts) plus native document bytes via narrow `study_files_*` commands (no desktop OPFS). `studyStoragePlatform.ts` centrally selects the adapter via `createStudyStorage()`; every multi-store browser transaction maps to one atomic `study_native_batch` SQLite transaction (CAS conditions + one-active-mock uniqueness guard). Desktop search uses a main-thread native bootstrap (`studySearchNativeBridge.ts`, pure `studySearchIndexCore.ts`, backend chosen by `studySearchPlatform.ts`): the worker builds/restores indexes from supplied records only and never touches IndexedDB, persisting fresh artifacts back over native IPC. Legacy Phase 1 IndexedDB/OPFS migration is deferred. Known limits: SQLite records plus file bytes are not a single atomic unit, and there is no live Windows/GUI validation yet. Phase 3B–3E adds native backup Open/Save dialogs behind the `studyFileInteractions.ts` boundary (coupled Rust `study_backup_*` commands; browser import/export unchanged). Import is parse-before-replace: the dialog-read text goes through `parseStudyImport` first and `replaceAll` runs only on success, so a failed import leaves existing data untouched; there is no staging area or recovery journal — the previous export file is the recovery path. See `study-desktop/README.md`.
...
The document reader loads only the selected document's components. The Study Unit editor, preview page, generated-missing-content action, and source-review acknowledgement load only the linked source keys for the active unit. Full-document component reads (`getLegalComponentsByDocument` and everything that derives from it) are deduplicated through a per-storage-instance in-memory cache (`study-desktop/src/studyDocumentCache.ts`): concurrent requests for one document share a single in-flight IndexedDB read and later requests resolve from memory until the exact seams that rewrite `legalComponents` (`replaceAll` — import/delete-all — and `importOfficialContentPackage`) invalidate it. Single-record reads (`getLegalComponent`, `getLegalComponentsBySourceKeys`) stay uncached so source-review acknowledgements always see fresh rows; the cache is a best-effort session perf layer, not a durability boundary.
...
Metadata is stored in `searchIndexMetadata` and serialized MiniSearch artifacts are stored in `searchIndexArtifacts` as `official-fulltext` and `study-fulltext` on the browser IndexedDB path. On the Tauri desktop path the same shapes live as native SQLite records supplied/persisted through the main-thread native bootstrap; the worker never opens IndexedDB there. `SEARCH_INDEX_SCHEMA_VERSION` and the MiniSearch package version are part of invalidation. If deserialization fails, the worker logs a development diagnostic, clears derived artifacts, and rebuilds from authoritative IndexedDB records. The Study storage layer and the browser search persistence open the SAME IndexedDB database at the SAME open version from the shared `study-desktop/src/studyDbConfig.ts` (name `webnet.study.v1`, version 10, schema version 10), and browser search metadata records that `dbVersion`, so the readonly browser search opener can never silently run against an older store layout and stale metadata is rebuilt (drift guard). On Tauri the native bridge records the native schema version instead and the worker never opens IndexedDB. The search worker's snapshot builder is v10-shaped (carries the five Exam Prep stores). Library search surfaces explicit preparing/loading progress and an unavailable/error banner when the worker fails.

`study-desktop/src/search/studySearchWorker.ts` initializes lazily when Library search is opened, loads valid persisted artifacts when possible, rebuilds stale/missing indexes off the React thread, keeps warm indexes in the worker, and returns compact result summaries containing IDs, display labels, scores, and worker-generated matched excerpts. Search scopes support All, Documents, Official Provisions, and Study Units. Request IDs prevent stale worker results from replacing newer query results.

Search index schema version `2` stores derived snippet text inside the serialized artifact so official-provision snippets can be built around the actual hit without fetching full source components into React. Official-provision records carry an explicit parent/document title field (`documentTitle`, indexed/stored/summarized separately — citation is never overloaded with the parent title), so parent-title terms match and result summaries expose the owning document title. Exact phrase matches such as `Director of Surveys`, exact title/citation matches, exact section searches, heading/title matches, and AND-term matches receive ranking boosts before incidental full-text OR matches. Development builds show compact Library search diagnostics with derived artifact size and official/Study record counts.

Phase 4B bulk generation can use `beginStudySearchBulkUpdate()` from `study-desktop/src/search/studySearchBulkIndexing.ts` to queue many upsert/remove unit IDs and commit one worker update. The current commit path rebuilds and persists the Study-content index once from authoritative IndexedDB records, avoiding thousands of separate serializations during bulk generation while preserving the official legal index.
...
- The sidebar item is named **Exam Prep** and routes to `/study/exam-prep`; tabs are Home, Learn (`/study/learn`), Recall (`/study/review`), Recognition (`/study/recognition`), Locate (`/study/locate`), Lookup Drills (`/study/drills`), and Mock Exam (`/study/mock-exam`). The legacy `/study/exam-curriculum` path renders Learn, preserving old bookmarks.
- Learn renders exactly the 133 A-D/NAV curriculum units with an independent studied/not-studied toggle. Home is the Phase 2.5 learner-workflow/readiness page: a compact **Suggested study flow** explains how the five modes fit together, a **Recommended Now** block picks the deterministic advisory next action (due Recall → unstudied Learn → missed Recognition → missed Locate → Developing drill → Unattempted drill → Accurate drill → Recognition practice; purely derived, never writes state), five independent readiness dimensions show Learn X/133 studied, Recall due + introduced X/57, Recognition coverage X/317 + latest-result accuracy, Locate coverage X/452 + latest-result accuracy, and Lookup Drills exam-ready X/24 + attempted X/24 (never combined into one percentage), a **Today's Activity** panel counts current-binding records on the local calendar date (zero state message when empty), the all-studied note (“All 133 curriculum units are marked studied.” — never “mastered”), and a **What these measure** help block keeps the labels honest.
- Exactly 57 recall cards are derived purely and deterministically from `unit.mustRecall` in canonical manifest order (A 18 / B 27 / C 6 / D 0 / NAV 6 / DRILL 0), with ids `recall:{unitId}:{index}`, the fixed prompt "State the key rule you should remember for this curriculum unit.", and verbatim frozen expected answers. Each derived task also carries the source unit's `reviewWeight` and `curriculumIndex` (position in the manifest units array) without changing ids or counts. mustLocate, sourceAnchors, recognitionCues, and drill answers never create cards, so all 452 mustLocate entries stay unscheduled.
- Recall review reuses the Study FSRS scheduler/config and runs **explicit frozen sessions**: pressing Start Recall Session freezes the session candidates exactly once (due cards earliest-first, then prioritized NEW cards — new-card rank order is exactly high A, high B, high NAV, other A, other B, other NAV, C, D with equal-rank ties by curriculum index, mustRecall index, then task id — capped by the new/max session limits, defaults 8/20, bounds 0-57/1-57, new <= max, excluding future-due and archived same-curriculum/different-hash records, and treating uninitialized progress as new). Successful ratings persist to the parent snapshot (immutable attempt + recall progress atomically with stale `updatedAt` protection) and advance through the frozen session; ratings never refill or reshape the running session. Failed/stale persistence keeps the revealed card and typed answer without advancing. After the final frozen card the view shows Session complete with an explicit Start Another Session button that freezes the next batch from the updated snapshot. Expected text and ratings stay hidden until Reveal.
...
### Exam Prep Phase 2.5 learner workflow / readiness UX

Phase 2.5 is a learner-experience pass only: **no curriculum content changes, no Recall/Recognition/Locate queue or FSRS changes, no drill-readiness rule changes, no DB/schema migration, and no new persistence** (schema stays v9 with exactly four Exam Prep stores; all new state is derived at runtime or is component-local filter state). Three pure helper modules provide the new derived behavior:

- `examPrepToday.ts` — `buildExamPrepTodayActivity(unitProgress, attempts, now)` counts today's activity on the machine-local calendar date using the shared local-date helper (never UTC slicing): studied units from `ExamPrepUnitProgress.studiedAt`, recall reviews from `ExamPrepRecallAttempt.reviewedAt`, recognition/locate from `completedAt`, and drill attempts by the persisted browser-local `practiceDate`. Every count is current-binding only, so archived same-curriculum/different-hash and other-curriculum records dated today never appear.
- `examPrepRecommendations.ts` — `selectExamPrepRecommendedAction(...)` answers **Recommended Now** with the fixed chain due Recall → unstudied Learn → missed Recognition (latest result per task) → missed Locate → Developing drill → Unattempted drill → Accurate drill → generic Recognition practice. `selectRecommendedExamPrepDrill(...)` picks the top drill needing work: status Developing → Unattempted → Accurate (Exam-ready is never recommended), then review weight high → medium → low, then canonical DRILL order. Both are deterministic, purely advisory, and ignore archived/other-curriculum attempts. The Learn pick reuses the pre-existing `selectRecommendedLearnUnit` rank philosophy and never auto-marks anything studied.
- `examPrepDrillFilters.ts` — canonical 24-drill readiness rows and the two local filter dimensions used by the Lookup Drills view: status (`All`, `Needs Work`, `Unattempted`, `Developing`, `Accurate`, `Exam-ready` — `Needs Work` is exactly Developing + Accurate) and difficulty (`All`, `Direct`, `Routing`, `Cross-document`), plus summary counts and per-status readiness reasons.

UI behavior: Home shows the Suggested study flow, Recommended Now (neutral copy such as “7 recall cards are due.” / “Next high-priority unstudied curriculum unit.” / “Status: Developing”), five independent readiness dimensions, Today's Activity (with “No Exam Prep activity recorded today yet.” when empty), the all-studied Curriculum coverage note, and the What-these-measure help block. Lookup Drills shows a readiness summary with the note “Exam-ready = 3/3 within the target time on two different practice dates.”, a Recommended drill block whose Go to drill resets any hiding filters and scrolls the card into view, and combinable status + difficulty filters (component-local, nothing persisted). Locate sprint copy now says “find the correct statute or controlling provision” (the frozen split is 390 provision-pinned vs 62 document-level targets; document-level reveals keep showing the statute title plus “Document-level target — no single provision is pinned in the curriculum.” and no deep link). Recognition reveal shows the curriculum unit id (`NAV-05`) and title instead of the internal `recognition:NAV-05:1` task id, which is still what attempts persist. Drill header target badges render M:SS via `formatExamDrillTime` (150 s → 2:30). Recognition and Locate sprint start screens state that the sprint samples the full exam curriculum (scope toggles intentionally deferred).

### Exam Prep Phase 3 — Configurable Provisional Mock Exam (schema v10)

Phase 3 adds one deliberately versioned **provisional** mock exam profile plus an engine that stays identical if the registrar later changes the format. The provisional profile `nb-statute-provisional-v1` configures 150 minutes, 30 mixed questions (6 Recall × 1pt, 8 Recognition × 1pt, 10 Locate × 1pt, 6 Lookup Drills × 3pt = 42 self-assessed points), open-book resources with the built-in browser statute library, `hard_stop` time policy, and `passMarkPercent = null` — every UI using it is labelled provisional and never claims an official ANBLS format or pass mark. Profiles live in the registry at `study-desktop/src/examPrep/mock/examPrepMockProfiles.ts`; a future registrar change is a config/data change (`nb-statute-official-v1`, `provisional-v2`, …) — the engine, session persistence, question pools, and results architecture are unchanged.

- **History-blind deterministic paper** (`examPrepMockPaper.ts`): given the same profile + seed, the same paper is produced for every learner (same question ids `q01..`, same source task ids, same mixed order). Selection uses only frozen pools — the 57 Recall tasks, 317 Recognition cues, 452 Locate targets, and 24 DRILL units — with a two-pass breadth rule (prefer each curriculum unit once across the non-drill portion) and profile-configured tier/difficulty quotas (A2/B2/NAV1/C1 Recall, A3/B2/NAV2/CD1 Recognition, A4/B3/NAV2/CD1 Locate, Direct 2 / Routing 2 / Cross-document 2 Drills; cycling fallback for profiles without a configured mix). `Math.random()` never runs inside the builder; a new session generates its seed once via `crypto.getRandomValues`.
- **One store per mock**: sessions snapshot the full profile used at start (`profileSnapshot`), store the seed, deadline (`startedAt + durationMinutes`), question refs, responses and per-question self-grading, and CAS writes (`saveExamPrepMockSession({session, expectation})`, expectation absent/existing(updatedAt)) so two tabs can never silently overwrite each other. Creating a brand-new `in_progress` session additionally takes a **transactional one-active-current-mock lock** inside the same readwrite transaction: if any other current-binding `in_progress` session already exists, the create is rejected (exactly one current mock per curriculum binding; updating the existing session id keeps the ordinary CAS path).
- **Mock never touches training data**: a mock Recall question does not update FSRS, mock Recognition/Locate/Drill results do not create `examPrepAttempts`, and unit studied state/settings are untouched — mock persistence writes only `examPrepMockSessions`. This is pinned by a contamination regression.
- **Exam UI** (`/study/mock-exam`, Mock Exam tab): provisional start screen with profile numbers/assumptions and recent current-binding history (graded results, `Grading incomplete · N of 30 graded` with Continue Grading, and a separate Abandoned list). While a mock is `in_progress` the page uses a focused exam layout with the ordinary tabs hidden: deadline-based countdown (reload cannot reset the timer; no pause), free-text answers, flag/visited palette, hard-stop expiry that locks responses, `[ Open Statute Library ]` in a new tab (shown only when the session's `profileSnapshot.resources.builtInStatuteLibrary` is true), submit confirmation with answered/unanswered/flagged counts, and a debounced CAS-serialized autosave (immediate on navigation/flag/submit) whose Saved / Saving… / Save failed indicator reflects the real state and whose queue never clobbers a revision typed while a save is in flight (the newest revision is saved before the indicator returns to Saved).
- **Self-grading and results**: after submission, answers are frozen and the learner self-assesses each question against the exact frozen expected content (Recall expected rule, Recognition unit id/title + likely sources, Locate document/provision with document-level targets preserved honestly, Drills with the frozen required lookups/answer points/trap and the three 1-point checkboxes). Grading persists and may be resumed; Finalize Results unlocks only when all 30 questions have grading records. Results are labelled **Practice score** (e.g. 34 / 42 · 81%) with a per-type breakdown and full question review; provisional results state “No official pass mark is configured for this provisional profile.” — never Passed/Failed/Exam grade. Reported time used comes from `submittedAt`/`abandonedAt` clamped to the hard-stop deadline — never `gradedAt`, so later grading does not inflate the exam clock — and a drill counts as correct/complete in the breakdown only at its full 3/3 self-score (1/3 and 2/3 stay partial).
- **Fail-closed hardening**: profiles are structurally validated before paper generation (`validateExamPrepMockProfile` in `examPrepMockProfiles.ts`: valid kind/bucket keys, non-negative integer counts, per-kind `questionMix` sums equal the kind's question count, positive duration/points, library-flag/open-book consistency) and every built paper must satisfy explicit postconditions in `examPrepMockPaper.ts` (question total, point total, per-kind quotas, sequential unique `qNN` ids, unique `sourceTaskId`s, per-question `pointsPossible` matching the profile) or the build throws instead of silently producing a wrong-size paper.
- **Bookkeeping**: Study DB and Study schema versions are now `10`; the store set is exactly five Exam Prep stores. `StudyDataSnapshot` gained `examPrepMockSessions` through seed/migrate/loadAll/replaceAll/reset/export/import; v9 databases and v9 JSON snapshots migrate additively with an empty mock history. Historical sessions keep their profile snapshot, so later profile-config changes never reinterpret old results. Mock selectors and results helpers are pure (`examPrepMockSelectors.ts`, `examPrepMockResults.ts`); immutable session updates live in `examPrepMockSession.ts`.

### Exam Prep learner QA / retrieval workflow

A learner-experience + QA pass (schema stays v10, exactly five Exam Prep stores; frozen curriculum/corpus untouched):

- **Shared DB config**: `study-desktop/src/studyDbConfig.ts` is the single source of `STUDY_DB_NAME`/`STUDY_DB_VERSION`/`STUDY_SCHEMA_VERSION`; `studyStorage.ts` re-exports them and `search/studySearchPersistence.ts` opens the same name at the same version (was a private 7). Search metadata records `dbVersion`; `metadataIsCurrent` requires it, so search indexes built against a different open version are treated as stale and rebuilt. The search worker snapshot is v10-shaped (schema + five Exam Prep stores).
- **Document load cache**: `studyDocumentCache.ts` caches `getLegalComponentsByDocument` per storage instance with in-flight dedupe; invalidated by `replaceAll` and `importOfficialContentPackage` (the only seams that rewrite `legalComponents`).
- **Safe new-tab links**: `studyWindow.ts` centralizes `noopener,noreferrer` new-tab opening. Its contract does NOT treat a null WindowProxy as a popup failure (with `noopener` the browser returns no handle even when the tab opened) — `openStudyUrlNewTab(path): StudyNewTabResult` reports `{ attempted: true }` after the open was requested and only surfaces genuine synchronous exceptions; callers never show a false “could not open” error from a null return. Active-session source links (Locate reveal exact provision, Drill reveal required lookups, Mock grading/results expected blocks, Locate/Mock Open Statute Library) open in a NEW tab through `EXAM_PREP_OPEN_SOURCE_BUTTON newTab` / `openStudyUrlNewTab`, so an active Locate/Drill/Mock never loses component-local state; Learn list chips keep in-SPA navigation.
- **Ephemeral Locate picker (browser-only)**: `examPrepLocatePicker.ts` defines the `?studyPicker=locate&prompt=…&token=…` protocol. The sprint tab opens the Library in a new tab with `noopener,noreferrer` and listens on a **token-scoped BroadcastChannel** (`webnet-study-locate-picker:<token>`) — the channel name carries only the random session token, never any answer/document data. Picker-mode Library and document-reader pages post `study-locate-pick` messages over that channel: search results carry `[ Use this document ]` (posts `documentId` with `sourceKey: null`) and `[ Use this provision ]` (posts the exact `sourceKey`) action buttons, opened documents offer `Select this provision` on components and expanded subsections, and every picker-mode in-tab navigation preserves the picker query. The sprint token-guards and objectively scores picks (`documentId` + exact pinned `sourceKey`; document-level targets accept any provision in the right statute), then PAUSES on feedback before persisting: the learner's selection is shown beside the frozen expected location and nothing is saved or advanced until `Continue`. Browsers without BroadcastChannel degrade gracefully (picker says automatic return is unavailable; Check Answer remains the fallback). Stale tokens/duplicate deliveries are ignored, the channel closes after one valid pick and on item change/sprint end/unmount, and Check Answer + self-report remain the always-available manual fallback.
- **Related units & Learn anchors**: unit cards expose `exam-unit-<id>` DOM ids; titled related chips (resolved against the bundled manifest) appear on Learn cards and drill reveals; Learn clears tier filters then scrolls/focuses `#exam-unit-…` hash targets on mount and hashchange; drill chips navigate `/study/learn#exam-unit-…`.
- **Recognition wording**: `examPrepRecognitionAskForTier`/`examPrepRecognitionExpectedLabelForTier` (in `examPrepConstants.ts`) make Recognition copy task-aware and single-sourced across the Recognition sprint, Mock pre-submission prompt, Mock grading echo, and Home flow copy: ordinary A/B/C/D cues ask “Which statute, regulation, bylaw, or legal topic should you check first?” (reveal heading `Expected topic`), while tier-NAV routing cues ask “What routing issue should this cue make you resolve first?” (reveal heading `Expected routing issue`). Frozen cues/scoring are untouched.
- **Recall reveal persistence**: after Reveal the prompt and typed answer stay visible with the expected answer and FSRS rating row until the learner rates; unrelated parent snapshot updates never collapse the reveal.
- **QA reports**: `npm run study:exam-prep:qa-report` builds byte-deterministic `reports/exam-prep/recall-quality-audit.md` (all 57 cards) and `reports/exam-prep/recognition-quality-audit.md` (all 317 cues) from the frozen pools. Recognition flags now include short NAV routing cues (≤3 words), generic legal nouns (exact normalized match only — longer cues that merely contain the word are not flagged), duplicate cues across different expected units, plus the existing very-short/long/multi-document flags, all summarized. The Recall report keeps the A-SURV-03 human-review note (three fragmented recall entries documented verbatim — never rewritten). Pure builder: `study-desktop/src/examPrep/qa/examPrepQaAudit.ts`.
- **Recall authoring review bundle**: `npm run study:exam-prep:recall-authoring-review` writes byte-deterministic `reports/exam-prep/recall-authoring-review.json` + `.md` (audit/authoring-PREPARATION only — no curriculum rewrite; the existing `recall-quality-audit.md` stays byte-identical and separate). Pure `qa/examPrepRecallAuthoringReview.ts` derives exactly 57 records (one per frozen card in canonical order; nothing invented) with current-card, owning-unit context (exam goal/depths/core/cues), source context (documents + titles, every owning-unit source anchor with document title/label/role, same-unit mustLocate targets), relationships (related ids, unit mustRecall count/index, sibling ids), and mechanical human-review flags (genericPrompt 57, multiRecallUnit 18, fragmentLike 10, longAnswer >240 = 5, veryLongAnswer >320 = 0, numericDetail 6, exceptionLanguage 11, multipleConcepts 27, answerOverlapsLocate 5 — fragment/long reuse the existing QA thresholds). Every JSON record carries empty human-authoring fields (`decision`/`proposedPrompt`/`proposedAnswer`/`proposedAction`/`reviewNotes` all null). `qa/examPrepRecallAuthoringReport.ts` renders the Markdown (group summary with counts + affected ids for triage, one `## NN — recall:…` section per card with a HUMAN REVIEW template, then a derived Multi-card units appendix including A-NBLS-01 and A-SURV-03). Metadata carries frozen `baselineCommit 36498a2…`, curriculum/manifest/corpus hashes, no wall clock. No runtime import; script only under `scripts/studyExamPrepRecallAuthoringReview.ts`.

### Exam Prep retrieval-workflow corrective patch

A focused functional correction (schema stays v10, five Exam Prep stores; frozen curriculum/corpus and all pool sizes untouched): the Locate picker now communicates over a **token-scoped BroadcastChannel** instead of `window.opener.postMessage` (which `noopener,noreferrer` removes), with channel names carrying only the random token, channel close/cleanup on pick/item/sprint/unmount, and a graceful unsupported-browser fallback; objective picks now **pause on feedback** (selection vs expected, correct/not-quite) and persist + advance ONLY on `Continue` — save failures keep the feedback screen for retry — while manual Check Answer stays intact; `openStudyUrlNewTab` no longer treats a null WindowProxy as a popup failure (no false “Could not open…” messages), showing “Locate picker opened in a new tab” after initiating the open; Recognition wording is task-aware (normal vs NAV) and shared by the sprint and Mock prompt/grading/review with expected-label presentation; the Recognition QA report gained short-NAV, generic-noun and cross-unit duplicate flags/summary. No curriculum text, IDs, queues, FSRS, Mock profile, DB/schema/store, or Recall QA content changed.
...
- The adapter exposes Study-facing `again`, `hard`, `good`, and `easy` ratings and maps them directly to the installed `Rating` enum inside `study-desktop/src/fsrs/studyFsrs.ts`.
