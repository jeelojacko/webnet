...
### Exam Prep Phase 3 — Configurable Mock Exam (schema v10)

Phase 3 adds one deliberately versioned **practice** mock exam profile plus an engine that stays identical if the registrar later changes the format. The practice profile `nb-statute-provisional-v1` configures 150 minutes, 30 mixed questions (6 Recall × 1pt, 8 Recognition × 1pt, 10 Locate × 1pt, 6 Lookup Drills × 3pt = 42 self-assessed points), open-book resources with the built-in browser statute library, `hard_stop` time policy, and `passMarkPercent = null` — every UI using it is labelled as practice and never claims an official ANBLS format or pass mark. Profiles live in the registry at `study-desktop/src/examPrep/mock/examPrepMockProfiles.ts`; a future registrar change is a config/data change (`nb-statute-official-v1`, `provisional-v2`, …) — the engine, session persistence, question pools, and results architecture are unchanged.
