...
- Learn renders exactly the 133 A-D/NAV curriculum units with an independent studied/not-studied toggle. Home shows studied X/133, recall due now, introduced X/57, the 57-card total plus current session limits, and a deterministic **recommended next unit** (the highest-ranked unstudied Learn unit — same rank philosophy as new recall cards: high A, high B, high NAV, other A, other B, other NAV, C, D, then manifest order); the recommendation is purely advisory and never auto-marks the unit studied.
