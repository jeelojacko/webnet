...
- `useMapViewShellEffects.ts`, `useMapViewObstacleFetch.ts`, `useMapView2dRenderState.ts`, and `useMapViewDragInteractions.ts` own extracted viewport/container effects, OSM obstacle fetch lifecycle, the 2D derived/render-state bridge, and drag/wheel pointer interactions so `MapViewRoot.tsx` can continue shrinking toward the source-size target without changing map behavior
