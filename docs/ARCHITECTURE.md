...
- `src/engine/cad/cadGeometry.ts` owns reusable 2D CAD point/line geometry helpers such as inverse, midpoint, segment nearest-point, intersection, and azimuth-distance resolution, while `src/engine/cad/cadGeometryCurves.ts` owns curve metrics, circle/arc intersections, tangent helpers, arc builders, and offset/parallel/perpendicular line geometry
