...
- `studyDbConfig.ts` for the single source of Study IndexedDB identity/version (`STUDY_DB_NAME`/`STUDY_DB_VERSION`/`STUDY_SCHEMA_VERSION`, all 10) shared by `studyStorage.ts` (re-export preserved) and the search worker's `studySearchPersistence.ts`, which opens the same name at the same version and records `dbVersion` in search metadata so index metadata can never claim currency across an open-version drift
- `studyStorage.ts` for IndexedDB-backed metadata, progress, attempt, draft, scheduler-state, settings storage, on-demand legal-component repository lookups, and atomic rating/undo writes; full-document component reads flow through `studyDocumentCache.ts` (`StudyDocumentComponentCache`, per-storage-instance in-memory cache with in-flight dedupe, invalidated inside `replaceAll` and `importOfficialContentPackage` — the two seams that rewrite `legalComponents`)
...
- `study-desktop/src/search/*` for the Phase 4A.1 derived Study search boundary: compact official/Study search records, MiniSearch serialization, IndexedDB-derived artifact persistence, worker messages, worker rebuild/query execution, and the React-facing search service
