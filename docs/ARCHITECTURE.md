...
- `parse.ts`
- `adjust.ts`, with top-level solver constants and internal engine/GPS/bootstrap types split into `adjustConstants.ts` and `adjustTypes.ts`, normal-equation helpers split into `adjustNormalEquationHelpers.ts`, GPS vector/frame helpers split into `adjustGpsVectorHelpers.ts`, observation weighting helpers split into `adjustObservationWeighting.ts`, robust-weight helpers split into `adjustRobustWeights.ts`, network/direction prefit diagnostics split into `adjustNetworkDiagnostics.ts`, direction diagnostic assembly split into `adjustmentDirectionDiagnostics.ts`, TS-correlation residual diagnostics split into `adjustmentTsCorrelationDiagnostics.ts`, weak float-zenith display projection split into `adjustmentWeakFloatZenithProjection.ts`, run-mode compatibility helpers split into `adjustmentRunModeCompatibility.ts`, and post-adjust sideshot result assembly split into `adjustmentSideshots.ts`
