...
- `src/study/content/nbSitCorpus*` plus `scripts/studyCorpus.ts` for the Phase 4A NB SIT statute-corpus manifest, manual-entry to current-source mappings, inventory validation, resumable official-source fetch, ANBLS PDF raw-source handling, normalization, integrity/package reports, source-change reports, and coverage outputs
...
- `runSession.ts`
- `solveEngine.ts`, with correction-only `normalEquationSolver` and equation-level `sparseCorrectionSolver` test seams in `numericalBackend.ts`; sparse packing lives in `sparseEquationPacking.ts`, and lazy WASM implementations live in `wasm/wasmModule.ts`, `wasm/wasmDenseNormalSolver.ts`, and `wasm/wasmSparseNormalSolver.ts`; covariance remains TypeScript
