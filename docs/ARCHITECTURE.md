...
- `src/study/examCurriculum/*` for the separate open-book Exam Curriculum V1 layer: schema/types (specs carry an optional `tier` field defaulting to `'A'`), the curated Tier-A catalog (51 units, 12 families) plus the Tier-B catalog split by subject area — `examCurriculumCatalogTierBLand.ts` (12 units), `examCurriculumCatalogTierBProperty.ts` (17), `examCurriculumCatalogTierBResources.ts` (7), `examCurriculumCatalogTierBPublic.ts` (7) — combined in `examCurriculumCatalogTierB.ts` (`examCurriculumTierBSpecs` = 43, `examCurriculumAllSpecs` = 94 with Tier A first, canonical family order, expected-count map, Tier-B document IDs/titles), deterministic corpus anchor resolution (document-order section ranges, schedule/form lookup), the educational-completeness validator (strict Tier-A contract; lighter Tier-B contract: empty `examGoal` allowed, orientation requires at least one payload array), the canonical manifest builder with content hashing plus per-tier projection hashes and per-tier metrics, and the bundled read-only `/study/exam-curriculum` page with an All / Tier A / Tier B filter. Node-only builds (`scripts/studyExamCurriculumBuild.ts`) resolve anchors against `study-content/packages/nb-sit-statute-corpus.content-package.json`; the layer shares no runtime state with the AI authoring or Study Map pipelines
