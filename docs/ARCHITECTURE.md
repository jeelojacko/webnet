...
Study Library search uses derived data. The official legal index is tied to `corpusContentHash`, the Study-content index is tied to `studyContentRevision`, and both serialized MiniSearch artifacts live in IndexedDB stores separate from authoritative records. The search worker owns index deserialization, corruption recovery, rebuild progress, scoped search, result limiting, and compact result summaries so React does not scan the full legal corpus on every keystroke.
