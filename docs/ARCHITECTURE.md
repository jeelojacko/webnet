...
- `src/study/examCurriculum/*` for the separate open-book Exam Curriculum V1 layer: schema/types, the curated Tier-A catalog specs, deterministic corpus anchor resolution (document-order section ranges, schedule/form lookup), the educational-completeness validator, the canonical manifest builder with content hashing, and the bundled read-only `/study/exam-curriculum` page. Node-only builds (`scripts/studyExamCurriculumBuild.ts`) resolve anchors against `study-content/packages/nb-sit-statute-corpus.content-package.json`; the layer shares no runtime state with the AI authoring or Study Map pipelines
