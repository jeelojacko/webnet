...
- `runSession.ts`
- `solveEngine.ts`, with the correction-only `normalEquationSolver` injection seam in `numericalBackend.ts`, and lazy WASM implementation in `wasm/wasmModule.ts` + `wasm/wasmDenseNormalSolver.ts`; covariance remains TypeScript
