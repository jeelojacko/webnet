...
- `src/engine/cad/cadCogo.ts` owns the first explicit COGO helper layer for inverse summaries, survey bearing formatting, bearing-distance point creation, line-like intersections, core point-line calculators (turned/deflection/along/extend/offset), curve metrics, and parcel reports
