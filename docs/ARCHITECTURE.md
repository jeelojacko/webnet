...
- `studyDbConfig.ts` for the single source of browser Study IndexedDB identity/version (`STUDY_DB_NAME`/`STUDY_DB_VERSION`/`STUDY_SCHEMA_VERSION`, all 10) shared by browser `studyStorage.ts` (re-export preserved) and the browser search worker's `studySearchPersistence.ts`, which opens the same name at the same version and records `dbVersion` in search metadata so index metadata can never claim currency across an open-version drift. The Tauri desktop runtime does not use this IndexedDB path; it uses Rust rusqlite native schema v1 (see Phase 2 note below)
- `studyStorage.ts` for browser IndexedDB-backed metadata, progress, attempt, draft, scheduler-state, settings storage, on-demand legal-component repository lookups, and atomic rating/undo writes; the Tauri desktop runtime instead uses `studyNativeStorage.ts` (same `StudyStorage` interface over narrow Rust SQLite IPC, selected centrally by `studyStoragePlatform.ts`/`createStudyStorage()`); full-document component reads flow through `studyDocumentCache.ts` (`StudyDocumentComponentCache`, per-storage-instance in-memory cache with in-flight dedupe, invalidated inside `replaceAll` and `importOfficialContentPackage` — the two seams that rewrite `legalComponents`)
- `studyWindow.ts` for centralized safe new-tab opening (`openStudyUrlNewTab` with `noopener,noreferrer`, popup-block `null`, `studyProvisionPath`) used by active Locate/Drill/Mock source links so component-local session state is never unmounted by a source deep-link
- `studyOpfs.ts` for OPFS source-file, normalized-text, imported-file, PDF, and backup path generation/writes
- `study-desktop/src/content/*` for the development-side official NB law manifest validation, laws.gnb URL generation, exact-text normalization models, and content-package validation
- `study-desktop/src/content/nbSitCorpus*` plus `scripts/studyCorpus.ts` for the Phase 4A NB SIT statute-corpus manifest, manual-entry to current-source mappings, inventory validation, resumable official-source fetch, ANBLS PDF raw-source handling, normalization, integrity/package reports, source-change reports, and coverage outputs
...
- `study-desktop/src/fsrs/*` for the Phase 3 FSRS adapter boundary, including Study-domain settings, serialized card/review-log records, runtime parameter validation, fixed/system clocks, legacy schedule migration helpers, replay predicates, and `ts-fsrs` scheduling calls isolated away from React components
- `study-desktop/src/search/*` for the Phase 4A.1 derived Study search boundary: compact official/Study search records, MiniSearch serialization, derived artifact persistence (browser: IndexedDB stores; Tauri desktop: native SQLite via main-thread bootstrap with no IndexedDB), worker messages, worker rebuild/query execution, and the React-facing search service
- `study-desktop/src/ai/*` for the Phase 4B.0 provider-neutral AI authoring boundary: Study Map jobs/results, unit proposals, learning objectives, grounding evidence, validation, reconciliation, and approval mapping into normal Study records
- `study-desktop/src/examCurriculum/*` for the separate open-book Exam Curriculum V1 layer: schema/types (specs carry an optional `tier` field defaulting to `'A'`), the curated Tier-A catalog (51 units, 12 families) plus the Tier-B catalog split by subject area — `examCurriculumCatalogTierBLand.ts` (12 units), `examCurriculumCatalogTierBProperty.ts` (17), `examCurriculumCatalogTierBResources.ts` (7), `examCurriculumCatalogTierBPublic.ts` (7) — combined in `examCurriculumCatalogTierB.ts` (`examCurriculumTierBSpecs` = 43, canonical family order, expected-count map, Tier-B document IDs/titles). Tier-C (21 units, 21 families — `examCurriculumCatalogTierC.ts`) and Tier-D (6 units, 6 families — `examCurriculumCatalogTierD.ts`) are composed in `examCurriculumCatalogTierCD.ts`. The cross-document Navigation layer lives in `examCurriculumCatalogNavigation.ts` (12 `ExamCrossDocumentNavigationSpec` units with multi-document `ExamCrossDocumentSourceSpec` sources and per-lookup `ExamCrossDocumentLookupSpec` targets); the lookup drill layer lives in `examCurriculumCatalogDrills.ts` (24 `ExamLookupDrillSpec` units: DRILL-01..08 direct, DRILL-09..16 routing, DRILL-17..24 cross_document, each with difficulty, timeTargetSeconds, factPattern, task, sources, answerKey with requiredLookups/requiredAnswerPoints/trapExplanation, relatedUnitIds). The canonical aggregator `examCurriculumCatalogAll.ts` exports `examCurriculumAllSpecs` = 157 (Tier A, then B, C, D, NAV, DRILL) and `EXAM_CURRICULUM_TOTAL` = 157, and the build script asserts 51/43/21/6/12/24 family/spec counts and corpus presence for all A-D document families plus every Navigation source document and all drill specifications. Deterministic corpus anchor resolution (document-order section ranges, schedule/form lookup), the educational-completeness validator (strict Tier-A contract; empty `examGoal` allowed for tiers B/C/D; Tier-C orientation requires cues + core with recognize/understand depths; Tier-D orientation is awareness-only requiring recognize plus a payload), the canonical manifest builder with content hashing plus per-tier projection hashes (A/B/C/D/NAV), per-tier metrics and the multi-source-aware `sourceCoverageByDocument` metric, a dedicated cross-document resolver dispatched from `buildExamCurriculumUnitFromCatalogSpec` (per-source resolution, `documentId::sourceKey` dedupe, `role: navigation` anchors, source-ordered `sourceDocumentIds`, explicit-document lookups) and the NAV-aware validator contract (separate 80-anchor breadth threshold), and the bundled read-only `/study/exam-curriculum` page with an All (157) / Tier A (51) / Tier B (43) / Tier C (21) / Tier D (6) / Navigation (12) / DRILL (24) filter. Node-only builds (`scripts/studyExamCurriculumBuild.ts`) resolve anchors against `study-content/packages/nb-sit-statute-corpus.content-package.json`; the layer shares no runtime state with the AI authoring or Study Map pipelines
- `study-desktop/src/examPrep/*` for the learner-facing Exam Prep layer over the frozen Exam Curriculum: manifest binding (one helper binds `curriculumId` + `curriculumContentHash` for every persistent record), pure recall/recognition/locate task derivation from `mustRecall`/`recognitionCues`/`mustLocate`, rank-priority helpers, frozen recall session queues, FSRS rating builders, session settings, immutable attempt builders/selectors, local-date activity, Recommended Now recommendations, drill filters/stats, and the tabbed Exam Prep page (Home / Learn / Recall / Recognition / Locate / Lookup Drills / Mock Exam) with components under `components/`. Phase 3 adds the `mock/` boundary — versioned profiles (`examPrepMockProfiles.ts`), deterministic history-blind paper generation (`examPrepMockPaper.ts` over the frozen pools), seeded randomness (`examPrepMockRandom.ts`), pure session builders/immutable updates (`examPrepMockSession.ts`), results/expiry helpers (`examPrepMockResults.ts`), current-binding session selectors (`examPrepMockSelectors.ts`), question content resolution (`examPrepMockQuestionContent.ts`), and focused UI components (active answering with CAS-serialized autosave + deadline timer, self-grading, results/history). Mock sessions are persisted in the v10 `examPrepMockSessions` store (schema/DB version 10; exactly five Exam Prep stores) and never touch FSRS, `examPrepAttempts`, unit studied state, or settings. Corrective hardening: per-record CAS writes plus a transactional one-active-current-mock creation lock inside `studyStorage.saveExamPrepMockSession` (a new `in_progress` session is rejected while any other current-binding `in_progress` session exists); the active-view autosave queue never discards a revision typed during an in-flight save and honestly shows Saving…/Saved; paper generation validates profiles/`questionMix` and asserts fail-closed paper postconditions; reported time used derives from `submittedAt`/`abandonedAt` clamped to the deadline (not `gradedAt`); a drill counts complete only at 3/3; the Open Statute Library affordance honours `profileSnapshot.resources.builtInStatuteLibrary`. Study routing is composed through `useStudyNavigation.ts`: it stamps `__webnetStudy` metadata on history entries, preserves full path/query/hash URLs and top-level caller state such as `returnTo`, and captures/restores the nested Study layout main scroll root; the global header Return is unavailable on direct loads and hidden during an active in-progress mock. Learner retrieval/QA additions: the Recognition ask line and practice description are shared constants (`examPrepConstants.ts`) consumed by the sprint, Mock prompt/grading, and Home; unit cards expose stable `exam-unit-<id>` anchors with titled related-unit chips (Learn scrolls `#exam-unit-…` deep links and clears tier filters first); the Locate sprint's ephemeral picker protocol lives in `examPrepLocatePicker.ts` (URL `?studyPicker=locate&prompt=…&token=…`, same-origin `study-locate-pick` messages, pure objective matcher) with picker-mode affordances rendered by `StudyLibrary.tsx` (`[ Use this document ]` / `[ Use this provision ]` search-result actions) and `StudyDocumentPage.tsx` (`Select this provision`), picker query preserved on every picker in-tab navigation, and the opener never persists or leaks the expected location; Recall reveal keeps the prompt, typed answer, expected answer, and FSRS ratings visible until rated; QA Markdown builders live in `study-desktop/src/examPrep/qa/examPrepQaAudit.ts` (pure, 57 recall + 317 recognition rows with mechanical flags) written by `scripts/studyExamPrepQaReport.ts` to `reports/exam-prep/*-quality-audit.md`.
...
The full NB SIT corpus pipeline is also Node-only. Its manifest lives at `study-content/manifests/nb-sit-statute-corpus.json`, raw snapshots at `study-content/raw/nb-sit/`, normalized documents at `study-content/normalized/nb-sit/`, and reports/packages under `study-content/reports/` and `study-content/packages/`. It reuses the existing NB law normalizer and package/import shape for `laws.gnb.ca` HTML sources. ANBLS PDFs are fetched and byte-hashed as professional-body raw sources, with embedded-text normalization guarded by document-specific English-only selectors.

Browser official-package import stores authoritative metadata in `legalDocuments` and exact legal text in `legalComponents`. Existing seeded `documents` keep user-authored summaries and are enriched by stable IDs rather than overwritten. Normal Study startup no longer reads all `legalComponents` into React state; document reading, unit editing, preview, source-review acknowledgement, and source-linked generation hydrate only the requested document or source keys through storage repository methods. Study units may carry source references keyed by legal `sourceKey` plus link-time content hash so later imports can mark source review without changing user-authored summaries, prompts, concepts, progress, attempts, or drafts.

Phase 2 native desktop persistence (Tauri shell; browser `/study` behavior unchanged): browser `/study` uses IndexedDB `webnet.study.v1` v10 plus OPFS `study/documents/...` assets. The Tauri desktop runtime uses Rust rusqlite native schema v1 at app-data `webnet-study/study.sqlite3` (one `(key, payload-JSON)` table per browser logical store, opaque derived search artifacts) plus native document bytes via narrow `study_files_*` commands (no desktop OPFS). `studyStoragePlatform.ts`/`studySearchPlatform.ts` centrally select browser vs Tauri adapters (`createStudyStorage()`); every multi-store browser transaction maps to one `study_native_batch` SQLite transaction (CAS + one-active-mock guard included). Desktop search runs the same worker with a main-thread native bootstrap (`studySearchNativeBridge.ts` + pure `studySearchIndexCore.ts`): records and cached artifacts load over native IPC, the worker never touches IndexedDB, and fresh artifacts persist back as `native-persist` over IPC. Legacy Phase 1 IndexedDB/OPFS migration is deferred (fresh native backends seed cleanly). Known limits: SQLite records and file bytes are not one atomic unit (write bytes first, read missing bytes fail-closed), and there is no live Windows/GUI validation yet. See `study-desktop/README.md`.

Study Library search uses derived data. The official legal index is tied to `corpusContentHash`, the Study-content index is tied to `studyContentRevision`, and both serialized MiniSearch artifacts live in IndexedDB stores separate from authoritative records. The search worker owns index deserialization, corruption recovery, rebuild progress, scoped search, result limiting, exact/AND-match ranking, matched-snippet generation, development diagnostics, and compact result summaries so React does not scan the full legal corpus on every keystroke. Phase 4B bulk generation can queue unit IDs through the Study search bulk-update API and persist one Study-content index update after a large generation batch.
...
Phase 4B AI authoring artifacts are stored separately from StudyUnits. `aiAuthoringRuns`, `aiStudyMapProposals`, and `aiUnitProposals` persist external Codex/ChatGPT/API-compatible authoring artifacts and review state. Phase 4B Study Map jobs default to the v3 prompt spec, while Unit Authoring jobs now default to v4. Jobs record prompt spec version in job identity, separate exact authoritative source from operative AI source, add bounded definition/cross-reference context, record source status/content flags/source-focus options, and require approved valid Study Map proposals before Unit Authoring jobs are prepared. Full-corpus Map preparation deterministically selects eligible section, schedule, and form targets from source structure and direct references, but external Study Map V3 authoring still owns dispositions, titles, learning goals, and grouping. Direct-reference context can resolve same-document sections, schedules, and forms. v3 Map results carry group `focusSelections`; reconciliation uses that focus to distinguish legitimate sibling splits from true source conflicts. Study Map grounding validation treats group `sourceKeys` as the authoring sources and previous/next/definition/direct-reference entries as understanding context only unless explicitly included in the group. Approval is explicit and maps one validated unit proposal into the existing StudyUnit, StudyPrompt, StudyRubricItem, StudyConcept, and initial progress records; proposals do not create attempts, FSRS review cards, or Library search records before approval. Validation emits blocking source/scope/grounding errors plus reviewable Map focus/priority/reference-only warnings and Unit coverage, overlap, full-approved-focus numeric/reference, modality, actor, answer-extension, and separate evidence-completeness warnings. v4 broad-group proposals can carry `mapRevisionSuggestion` feedback instead of defaulting to approvable StudyUnit content.
...
- `src/hooks/useAppWorkspaceDraft.ts` and `src/hooks/useWorkspaceProjectState.ts` keep the active standalone CAD drawing available in the browser workspace, while `src/hooks/useProjectFileWorkflow.ts` keeps adjustment `.wnproj` persistence CAD-free except for migration reads from older project files
