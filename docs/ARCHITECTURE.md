...
- during active middle-drag 2D panning, `MapView.tsx` now freezes the derived viewport/culling basis at the drag-start view while the live transform continues to move, so SVG/canvas/WebGL overlays pan as one stable slab and only refresh their visible-set derivation once the drag ends
