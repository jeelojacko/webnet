...
Phase 4B AI authoring artifacts are stored separately from StudyUnits. `aiAuthoringRuns`, `aiStudyMapProposals`, and `aiUnitProposals` persist external Codex/ChatGPT/API-compatible authoring artifacts and review state. Phase 4B.0.1 jobs default to v2 prompt specs, record prompt spec version in job identity, separate exact authoritative source from operative AI source, add bounded definition/cross-reference context, and require approved valid Study Map proposals before Unit Authoring jobs are prepared. Approval is explicit and maps one validated unit proposal into the existing StudyUnit, StudyPrompt, StudyRubricItem, StudyConcept, and initial progress records; proposals do not create attempts, FSRS review cards, or Library search records before approval. Validation emits blocking source/scope/grounding errors plus reviewable coverage, overlap, numeric, modality, actor, and evidence-extension warnings.
