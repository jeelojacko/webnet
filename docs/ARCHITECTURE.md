...
- `useMapViewShellEffects.ts`, `useMapViewObstacleFetch.ts`, and `useMapView2dRenderState.ts` own extracted viewport/container effects, OSM obstacle fetch lifecycle, and the 2D derived/render-state bridge so `MapViewRoot.tsx` can continue shrinking toward the source-size target without changing map behavior
