...
- `src/components/surveyCad/SurveyCadFloatingPanelShell.tsx` now owns the shared compact floating-panel chrome for Survey CAD popup surfaces, including drag header, dock/floating shell positioning, and persisted right/bottom/corner resize-handle seams so later CAD popups can reuse one panel pattern instead of bespoke fixed overlays
