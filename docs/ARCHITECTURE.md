...
- `parse.ts`
- `adjust.ts`, with top-level solver constants and internal engine/GPS/bootstrap types split into `adjustConstants.ts` and `adjustTypes.ts`, normal-equation helpers split into `adjustNormalEquationHelpers.ts`, GPS vector/frame helpers split into `adjustGpsVectorHelpers.ts`, GPS observation modeling split into `adjustGpsObservationModel.ts`, observation weighting helpers split into `adjustObservationWeighting.ts`, solve timing split into `adjustSolveTiming.ts`, main solve orchestration split into `adjustSolveWorkflow.ts`, data-check workflow helpers split into `adjustDataCheckWorkflow.ts`, coordinate-system warning helpers split into `adjustCoordSystemDiagnostics.ts`, geoid height helpers split into `adjustGeoidHeightHelpers.ts`, datum/grid input checks split into `adjustDatumChecks.ts`, bootstrap traverse approximation split into `adjustBootstrapHelpers.ts`, distance/reduction helpers split into `adjustReductionHelpers.ts`, robust-weight helpers split into `adjustRobustWeights.ts`, post-solve statistics and precision propagation split into `adjustStatistics.ts`, TS-correlation weight matrix helpers split into `adjustTsCorrelationWeights.ts`, network/direction prefit diagnostics split into `adjustNetworkDiagnostics.ts`, direction diagnostic assembly split into `adjustmentDirectionDiagnostics.ts`, TS-correlation residual diagnostics split into `adjustmentTsCorrelationDiagnostics.ts`, weak float-zenith display projection split into `adjustmentWeakFloatZenithProjection.ts`, run-mode compatibility helpers split into `adjustmentRunModeCompatibility.ts`, and post-adjust sideshot result assembly split into `adjustmentSideshots.ts`
