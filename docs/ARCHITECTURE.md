...
- `src/engine/cad/cadCogo.ts` remains the compatibility barrel for CAD COGO helpers, with primitive math/formatting/point-line/intersection/curve helpers split into `src/engine/cad/cadCogoMath.ts`; parcel compatibility exports remain in `src/engine/cad/cadCogoParcel.ts`, with shared parcel geometry/report/diagnostic helpers in `src/engine/cad/cadCogoParcelGeometry.ts`, line/bearing/area split solvers in `src/engine/cad/cadCogoParcelSplit.ts`, frontage-reference helpers in `src/engine/cad/cadCogoParcelFrontage.ts`, layout-local polygon clipping helpers in `src/engine/cad/cadCogoParcelLocalGeometry.ts`, shared layout result types in `src/engine/cad/cadCogoParcelLayoutTypes.ts`, frontage-path/slide/swing layout primitives in `src/engine/cad/cadCogoParcelLayoutPrimitives.ts`, parcel layout preview/evaluation helpers in `src/engine/cad/cadCogoParcelLayoutEvaluation.ts`, generated-draft/frontage-line utilities in `src/engine/cad/cadCogoParcelLayoutDrafts.ts`, generated-lot conflict resolution helpers in `src/engine/cad/cadCogoParcelLayoutConflicts.ts`, corner-frontage geometry helpers in `src/engine/cad/cadCogoParcelCornerGeometry.ts`, closed-boundary ring auto-layout in `src/engine/cad/cadCogoParcelClosedBoundary.ts`, auto-layout stabilization/remainder helpers in `src/engine/cad/cadCogoParcelAutoLayoutHelpers.ts`, supported-remainder automatic fill sequencing in `src/engine/cad/cadCogoParcelAutoLayoutSequence.ts`, frontage-path/strip auto-layout drafting in `src/engine/cad/cadCogoParcelFrontagePathAutoLayout.ts`, and single-frontage automatic layout orchestration in `src/engine/cad/cadCogoParcelAutoLayoutSingle.ts`
