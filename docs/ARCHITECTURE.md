...
- `src/study/examCurriculum/*` for the separate open-book Exam Curriculum V1 layer: schema/types (specs carry an optional `tier` field defaulting to `'A'`), the curated Tier-A catalog (51 units, 12 families) plus the Tier-B catalog split by subject area — `examCurriculumCatalogTierBLand.ts` (12 units), `examCurriculumCatalogTierBProperty.ts` (17), `examCurriculumCatalogTierBResources.ts` (7), `examCurriculumCatalogTierBPublic.ts` (7) — combined in `examCurriculumCatalogTierB.ts` (`examCurriculumTierBSpecs` = 43, canonical family order, expected-count map, Tier-B document IDs/titles). Tier-C (21 units, 21 families — `examCurriculumCatalogTierC.ts`) and Tier-D (6 units, 6 families — `examCurriculumCatalogTierD.ts`) are composed in `examCurriculumCatalogTierCD.ts`; `examCurriculumCatalogTierB.ts` exports the full `examCurriculumAllSpecs` = 121 (Tier A first, then B, C, D), and the build script asserts 51/43/21/6 family/spec counts and corpus presence for all 49 document families. Deterministic corpus anchor resolution (document-order section ranges, schedule/form lookup), the educational-completeness validator (strict Tier-A contract; empty `examGoal` allowed for tiers B/C/D; Tier-C orientation requires cues + core with recognize/understand depths; Tier-D orientation is awareness-only requiring recognize plus a payload), the canonical manifest builder with content hashing plus per-tier projection hashes and per-tier metrics, and the bundled read-only `/study/exam-curriculum` page with an All (121) / Tier A (51) / Tier B (43) / Tier C (21) / Tier D (6) filter. Node-only builds (`scripts/studyExamCurriculumBuild.ts`) resolve anchors against `study-content/packages/nb-sit-statute-corpus.content-package.json`; the layer shares no runtime state with the AI authoring or Study Map pipelines
