...
`GENERIC_MAIN_QUESTION` recognizes normalized templates such as `What should a learner remember about <approvedGroup>?`. `GENERIC_GUIDED_QUESTION` recognizes templates such as `What does <citation> require or allow for <approvedGroup>?` and `What is the rule for <approvedGroup>?`. `ANSWER_APPEARS_TRUNCATED` covers literal fixed-limit ellipses, source substrings that begin or end inside an alphanumeric token, suspicious lowercase starts, suspicious mid-sentence endings, evidence excerpts with the same defects, and summaries built from truncated objective answers. `DEFINITION_ANSWER_MISSING_TERM_MEANING` prevents a defined-term objective from validating on the generic definitions preamble. `DUPLICATE_NONRESPONSIVE_ANSWER` catches repeated answers across materially distinct objective questions in the same unit. `MAP_REVISION_UNSUPPORTED_CONCEPT` catches source-unsupported map revision concepts such as the Phase 4B.2 Community Planning Act s.125 invented "retroactive effect" split.
