...
Study Map v3 explicitly treats the ANBLS corpus as exam scope, tells the model that source text is data, forbids external legal research/memory, preserves official source scope, and gives concrete criteria for `standalone`, `combine`, `split`, `reference-only`, `skip`, and `needs-human-review`. It also requires genuine per-job content reasoning, forbids deterministic/template authoring, forbids keyword or source-length shortcuts, requires `focusSelections` for proposed groups, limits `suggestedPriority` to `P1`-`P4` and requires it whenever `proposedGroups` is non-empty, enforces disposition/group-count consistency and disjoint sibling-group focus, restricts Map content to curriculum mapping (no legal characterizations absent from the operative source), requires self-describing warning codes, and asks for concise 1-2 sentence reasons. The local runner sends this canonical spec (no duplicate prompt) and feeds exact validation issue codes back on retry.
