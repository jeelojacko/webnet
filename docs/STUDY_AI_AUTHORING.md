...
Coverage gaps are keyed by `documentId::sourceKey` because legal source keys such as `section:1` repeat across documents. Queue C clean/high-confidence proposals do not become approved Map content automatically; they are sampled for human semantic audit.
