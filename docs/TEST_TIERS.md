...
| **evidence** | `npm run test:evidence [-- <suite>]` | `vitest.evidence.config.ts` via `scripts/runEvidence.mjs` | Only the intentionally expensive long numerical campaigns under `tests/evidence/`. Expected to be very slow; manual-only, never runs in CI. Suites: `all` (default), `phase8b1`, `phase8a6`, `phase8a5`, `phase9a` (all three Phase 9A shards), `phase9a-scaling`, `phase9a-faults`, `phase9a-corpus`, `phase9d`, `phase9e-audit`, `phase9e-fastpath`, `phase9h`, `phase9i`, `phase9j`, `phase10a`, `phase10b`, `phase10c`, `phase10d`, `phase10e`, `phase10f`, `phase10g`, `phase10h`, `phase10i`, `phase10j`, `phase10k`, `phase10l`, `phase10m`, `phase10n`; unknown names fail fast. |
...
- **Evidence** (`tests/evidence/phase8b1_preanalysis_release.test.ts`,
  ...
  `tests/evidence/phase10n_native_correction_evidence.test.ts`): large repeated
