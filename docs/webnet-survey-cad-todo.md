...
- [x] Survey CAD geometry curve split batch 23 (July 2026): split circle/arc intersection, tangent-point, and offset/parallel/perpendicular helpers into `cadGeometryCurveIntersections.ts`, while keeping `cadGeometry.ts` exports stable.
...
- [x] Survey CAD parcel COGO split batch 8 (July 2026): split corner-frontage geometry helpers into `cadCogoParcelCornerGeometry.ts`, while keeping compatibility exports stable.

- [x] Survey CAD parcel COGO split batch 9 (July 2026): split closed-boundary parcel ring auto-layout into `cadCogoParcelClosedBoundary.ts`, while keeping compatibility exports stable.

- [x] Survey CAD parcel COGO split batch 10 (July 2026): split parcel auto-layout stabilization/remainder helpers into `cadCogoParcelAutoLayoutHelpers.ts`, while keeping compatibility exports stable.

- [x] Survey CAD parcel COGO split batch 11 (July 2026): split supported-remainder automatic parcel fill sequencing into `cadCogoParcelAutoLayoutSequence.ts`, while keeping compatibility exports stable.

- [x] Survey CAD parcel COGO split batch 12 (July 2026): split frontage-path and frontage-strip parcel auto-layout drafting into `cadCogoParcelFrontagePathAutoLayout.ts`, while keeping compatibility exports stable.
...
- [x] Survey CAD frontage-reference auto-layout split batch 21 (July 2026): split generated-remainder replacement/fill helpers into `cadCogoParcelFrontageReferenceRemainders.ts`, while keeping frontage-reference auto-layout exports stable.
...
- [x] Survey CAD TypeScript structure enforcement batch 3 (July 2026): split CAD transaction metadata, linked-point/dependency sync, COGO report, and parcel-layout report helpers into focused transaction helper modules while preserving command behavior.

- [x] Survey CAD TypeScript structure enforcement batch 4 (July 2026): split CAD transaction selection expansion and entity transform/grip-edit helpers into focused transaction helper modules while preserving command behavior.

- [x] Survey CAD TypeScript structure enforcement batch 5 (July 2026): split CAD transaction TRIM/EXT common geometry plus trim/extend piece and preview helpers into focused modules, while preserving command behavior and public preview exports.
...
- [x] Survey CAD TypeScript structure enforcement batch 9 (July 2026): split parcel create, line split, bearing split, and area split transaction commands into focused modules while preserving registry behavior.

- [x] Survey CAD TypeScript structure enforcement batch 10 (July 2026): split polyline, traverse, and batch COGO transaction commands into a focused module while preserving registry behavior.

- [x] Survey CAD TypeScript structure enforcement batch 11 (July 2026): split curve creation commands and entity edit commands into focused modules while preserving registry behavior.
...
- [x] Survey CAD command-hook split batch 5 (July 2026): split snap construction-context building into `useSurveyCadCommandConstruction.ts`, while preserving command snapping context behavior.

- [x] Survey CAD command-hook split batch 6 (July 2026): split command preview-state building into `useSurveyCadCommandPreview.ts` and arc/curve preview helpers into `useSurveyCadCommandArcPreview.ts`, while preserving live preview behavior.

- [x] Survey CAD command-hook split batch 7 (July 2026): split selected-line command point adapters and active edit command target builders into `useSurveyCadCommandSelection.ts`, while preserving hook result shape.

- [x] Survey CAD command-hook split batch 8 (July 2026): split command starter construction into `useSurveyCadCommandStarters.ts`, while preserving command launch guards and session payloads.

- [x] Survey CAD command-hook split batch 9 (July 2026): split traverse draft row-editing, sideshot, rewind, close-loop, and adjustment actions into `useSurveyCadTraverseDraftActions.ts`, while preserving traverse draft behavior.

- [x] Survey CAD command-hook split batch 10 (July 2026): split command lifecycle handlers for finish, cancel, Enter/Escape, and batch COGO commit into `useSurveyCadCommandLifecycle.ts`, while preserving command completion behavior.

- [x] Survey CAD command-hook split batch 11 (July 2026): split command input edits, active snap consumption, batch COGO draft text updates, and pointer-interaction point adaptation into `useSurveyCadCommandInputActions.ts`, while preserving command input behavior.

- [x] Survey CAD command-hook split batch 12 (July 2026): split report publishing, batch COGO draft construction, and command availability flag builders into focused command helper modules, while preserving command report, draft, and result flag behavior.

- [x] Survey CAD command-hook split batch 13 (July 2026): split arc, edit/modify, and parcel-split point-pick consumers into focused command helper modules, while preserving point-pick command behavior.
...
- [x] Survey CAD command-hook split batch 16 (July 2026): split typed report, parcel split, construction-point, and alignment submit handlers into `useSurveyCadTypedSubmit.ts`, while preserving command reports, transactions, and point creation behavior.

- [x] Survey CAD command-hook split batch 17 (July 2026): split snap-fed point consumption into `useSurveyCadConsumePoint.ts` and remaining traverse/fillet/arc/basic typed-submit handling into `useSurveyCadDefaultSubmit.ts`, bringing `useSurveyCadCommands.ts` below the 600-line warning while preserving command behavior.
