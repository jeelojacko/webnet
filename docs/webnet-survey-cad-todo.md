...
- [ ] Survey CAD parcel-auto-layout batch (June 2026): add repeated frontage-aware parcel generation for one selected parent parcel, including preview-all vs step-through creation, automatic fill mode, deterministic parcel naming, and remainder handling options that can place leftover area in the last parcel, create a remainder parcel, or redistribute it.
  Current slice: extend automatic fill to swing layouts and staged `redistribute remainder` handling on top of the new slide-based preview-all, single-step accept, and fill-parent commit surfaces.
