...
- [x] Survey CAD parcel-auto-layout batch (June 2026): add repeated frontage-aware parcel generation for one selected parent parcel, including preview-all vs step-through creation, automatic fill mode, deterministic parcel naming, and remainder handling options that can place leftover area in the last parcel, create a remainder parcel, or redistribute it.
  Current slice: replace the current frontage-strip fallback with a corner-aware auto solver that refuses criss-crossed lots, enforces optional max-depth, scores candidates toward realistic min area/frontage/width/depth targets, carries surviving chained frontage by geometry, trims adjoining strip runs by the frontage actually consumed inside accepted corner lots, and now also tries max-depth-sized, redistributed, remainder-preserving, and area-biased corner candidates before the straight runs; the corner prepass now shortlist-scores candidates by actual merged junction yield with the adjoining straight strip runs instead of only raw trim heuristics, then retries a local corner fill inside the actual trailing remainder polygon when the prepass corner fails while staying on the selected frontage chain instead of inventing lots from non-selected parent-boundary edges, and can replace conflicting generic strip lots with a localized corner refill when that turns two corridor lots into three realistic corner lots, while only pushing generic failed-corner trim onto the downstream frontage edge and finally removing any duplicated corridor lots if two chained frontage passes still claim the same remainder strip, so skewed/tapered turn lots split finer and stop burning excessive frontage on the screenshot-style regression fixtures.
