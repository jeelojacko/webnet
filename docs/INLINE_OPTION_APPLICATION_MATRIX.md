...
| `.LWEIGHT`             | N/A                | Applied                                   | Applied                   | Applied (`L`)  | Applied (report/profile shows state) | Fallback weighting is wired for omitted-sigma leveling equations across `L` plus the length-aware delta-derived paths (`DV`/`M`/`BM`/`T`/`DM`/`SS`). Precedence is explicit sigma token first, then active inline/project `.LWEIGHT`, then instrument `mm/km` fallback. Pure `V` records remain lengthless and therefore do not gain a `mm/km` fallback term. |
