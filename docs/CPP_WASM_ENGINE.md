...
Safety benchmarks (`npm run bench:phase7b7:safety`) define S0 static-preflight+sparse, S1 first-system oracle, S2 first-two-system oracle, and S3 every-iteration oracle. On deterministic chain cases from 8 through 128 unknowns, all strategies had zero fallbacks; S3 oracle medians ranged from 0.12 ms at 8 to 15.45 ms at 128. The conservative proposed cap is 64 unknowns, where S3 overhead was 3.98 ms and candidate median was 11.38 ms; module initialization was measured separately at 2.15 ms median. Reports include worker timings, RSS/heap, and deterministic JSON/Markdown output under `reports/phase7b7/`. This is readiness evidence only: production remains TypeScript-only and Phase 7C routing was not started.

### Phase 8A.7 — preanalysis production hook (default-disabled; no behavior change)

Production plumbing only. The audited 8A.6 sentinel math is promoted verbatim into neutral production-safe `src/engine/preanalysisSparseCovarianceSentinel.ts` (packed N = A'WA accumulation, deterministic column probes, C1/C2/C3, physical validation) and `src/engine/preanalysisSparseSessionPolicy.ts` (unknown cap 128, planning-system cap 64, atomic all-or-TypeScript fallback), with no evidence-only, test-helper, or script imports. Sibling `src/workers/preanalysisSparseAutoRoute.ts` runs eligible preanalysis/2D/plain sessions (station unknowns <= 128) through the real WASM sparse bundle behind typed pre-dispatch gates (default OFF): `PreanalysisGatedCorrectionSolver` counts one planning system per native correction entry and `PreanalysisGatedCovarianceCapture` pairs each covariance call to a started system, both throwing typed `PreanalysisSparseCapError` BEFORE delegating past 64 systems or 256 runtime parameters (Phase 9B: station unknowns stay capped at 128 by eligibility/policy; the per-system parameter cap is the split `PREANALYSIS_SPARSE_ROUTE_MAX_PARAMETERS = 256`, threaded explicitly through correction, covariance header, and the bounded-verification sentinel (all using the 256 runtime parameter cap), so over-cap systems never execute natively (capture still bounded at 512, fail-closed truncation). Post-run damping/fallback/C1/C2/C3/physical gates judge captured native values plus one bounded deterministic all-pairs verification set per system (n^2 <= 16384), condition warn-only, no correction-oracle authority, pairing coverage required, and whole-session exactly-once atomic TypeScript restart from the original immutable request. Disabled short-circuits with no WASM init; test-only hooks (unknownCountOverride, systemCapOverride, forceC2Failure, forcePhysicalFailure) cover exact static 127/128/129, gate-unit 63/64/65 stop-at-64 and 255/256/257 not-delegated proofs (Phase 9B runtime boundary), live-session cap abort, and forced verification failures without protocol or user-facing-flag changes. `src/workers/adjustmentWorker.ts` dispatches preanalysis to the new route and adjustment to the untouched Phase 7C route (injected-runtime precedence and protocol unchanged), reusing the existing adjustment WASM loader/cache. Coverage in `tests/phase8a7_preanalysis_production.test.ts` (15 tests); reports under `reports/phase8a7/`. Phase 8B automatic routing stays NO-GO.

### Phase 9B — production runtime parameter cap 256 (caps split; no C++/UI/protocol/persistence changes)

Authoritative caps are now split: station unknowns 128 (static eligibility + whole-session policy), runtime per-system parameters 256 (correction pre-dispatch, covariance header/immediate-verify, bounded-verification sentinel via explicit caps), planning systems 64, verification k = 16, backstop 16384, captured 512. Historical Phase 9A committed evidence (`reports/phase9a/`, `tests/phase9a_release_verdict.test.ts`, `tests/evidence/phase9a_*`) is untouched; rollout evidence lives under `reports/phase9b/cap-256-rollout.{json,md}` with the 9A sha256 pinned. Fast gates: `tests/phase9b_preanalysis_parameter_cap.test.ts` (agent-required), `tests/phase9b_preanalysis_parameter_cap_realwasm.test.ts` (wasm, one compact session, skips without artifact), `tests/phase9b_release_verdict.test.ts` (release).

### Phase 9C — direction-heavy preflight routing
