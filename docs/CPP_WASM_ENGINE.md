...
Deterministic real-WebNet 3D generated cases now cover `gps-3d-16/32/64/128`, with `.3D`, mixed distance/bearing/leveling observations, GPS EN correlation, truth checks split into horizontal and height maxima, selected covariance, standardized residuals, repeat determinism, and zero-fallback assertions. The sparse-only quick run passed `gps-3d-16` (131 equations, 48 parameters, horizontal truth error `8.77e-4 m`, height `8.45e-4 m`); manageable TS-vs-sparse proofs passed for 16 and 32 unknown stations. Full ENU GPS covariance (`EU`/`NU`) and a dedicated 3D worker proof remain unclaimed because the generated G-record path currently exercises valid EN covariance only.

The condition estimate audit found a user-visible, non-parity contract: the existing iteration-1 value is the raw normal-matrix row/column norm product, shown in text results and the report warning panel. Sparse correction currently omits it. Before routing, either compute the same norm-product from sparse normal entries in O(nnz), or document and test an explicit equivalent; silently dropping the visible diagnostic is not acceptable.
...
`evaluateSparseProductionEligibility` is a pure, test-only, fail-closed classifier. It rejects unsupported run modes, missing WASM or worker availability, robust/TS-correlation/GPS-covariance modes, 3D, size overflow, and rank risk in deterministic reason order. It is not wired to routing. Production remains TypeScript-only. A test-only actual adjustment-worker protocol proof now loads `src/workers/adjustmentWorker.ts` under Node worker_threads, sends a real `RunRequestMessage`, exercises TypeScript assembly, validates queued/solving/finalizing/success responses, and passes deterministic repeat checks. The existing request protocol has no sparse-engine injection seam, so sparse numerical execution inside that worker remains intentionally unclaimed. Phase 7B is not started; the evidence-based recommendation remains internal shadow-before-routing, then a small/medium 2D worker candidate only after sparse condition semantics, all-pairs policy, dense-fallback safety, and full ENU/large-3D evidence are complete.
