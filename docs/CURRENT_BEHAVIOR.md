...
- the top toolbar now includes a `Survey CAD` launcher beside `Project Options`; today it opens a planning/workspace seam with branch-local roadmap links rather than production CAD editing tools
...
- a `Survey CAD` workspace tab that can be opened before any solve; it currently acts as the durable entry seam for CAD architecture docs, phased TODOs, and future implementation branches
