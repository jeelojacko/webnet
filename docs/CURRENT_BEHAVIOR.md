...
- 2D map derivation now caches base projected survey geometry separately, culls the current viewport in that base projected space first, and only then applies the deferred pan/zoom transform to the surviving subset; this keeps dense Camp-style zoom/pan sweeps from retransforming the full network each frame
