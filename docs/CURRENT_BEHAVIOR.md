...
- browser-only map pan regressions can now be reproduced on the dedicated `map-pan-harness.html` Camp startup page and checked with `npm run test:map-browser`, which drives the middle-mouse pan path in Chromium through Playwright instead of relying only on jsdom interaction tests
