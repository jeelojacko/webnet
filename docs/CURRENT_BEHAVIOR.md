...
- a `Survey CAD` workspace tab that can be opened before any solve; it currently acts as the durable entry seam for CAD architecture docs plus the live native-model/renderer spike preview, selection/undo command-history spike, snapping-status workflow, typed command-input workflow (`POINT`, `COGO PT`, `LINE`, `PLINE`, `INVERSE`, `MOVE`, `COPY`, `INTX`), and adapter-inspection workflow
