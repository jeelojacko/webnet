...
- the bounded preanalysis recommendation solve pool now pre-ranks remaining scenario families by weak-geometry overlap and synthetic-family priority before the expensive re-solves, so high-impact brace/promoted/synthetic scenarios can surface earlier instead of waiting behind many repeated existing-set trials
...
- the WebGL basemap renderer now caches warped tile mesh vertices per tile signature instead of rebuilding them every draw, and it also uses a coarser interaction-time tile zoom plus lighter interaction mesh density to keep OSM-on pans and zooms closer to the no-basemap path
- the 2D render surface is now aspect-locked and centered inside the `Map & Ellipses` pane instead of stretching to the container bounds, so collapsing the input sidebar or changing the workspace width no longer distorts survey geometry horizontally or vertically
...
- preanalysis recommendations still prefer templates touching the current weakest geometry first, but the bounded re-solve pool now hard-limits repeated existing-set trials whenever brace/promoted/synthetic/cross-tie options exist, and equal-score ordering breaks toward those synthetic families first; once the local weak-seed pool is exhausted they still fall back to all remaining feasible scenarios so operators can keep applying batches until the threshold is reached or every usable scenario has been consumed
