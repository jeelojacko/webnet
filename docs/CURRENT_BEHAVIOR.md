...
- preanalysis recommendations still prefer templates touching the current weakest geometry first, but once that local weak-seed pool is exhausted they fall back to all remaining feasible scenarios so operators can keep applying batches until the threshold is reached or every usable scenario has been consumed
