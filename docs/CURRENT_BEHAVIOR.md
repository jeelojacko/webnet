...
- the top toolbar now includes the only `Survey CAD` launcher beside `Project Options`; today it opens a dedicated full-page Survey CAD workspace that hides the input editor/results chrome while active, removes the duplicate results-area CAD tab, keeps the page trimmed to CAD toolbar plus model space, builds native CAD entities from current WebNet input or solved results, renders them through an internal SVG viewport, exposes native selection plus transaction-history seams (`Select All`, `Clear Selection`, `ERASE`, `Undo`, `Redo`), adds internal point-node / endpoint / midpoint / intersection / nearest snap feedback over the preview surface, supports typed/snap-driven `POINT`, `COGO PT`, `LINE`, `PLINE`, `ARC 3PT`, `TAN CURVE`, `INVERSE`, `MOVE`, `COPY`, and selection-driven `INTX` workflows with coordinate, azimuth-distance, survey bearing-distance, and tangent-curve radius entry plus inverse bearing readout, keeps the toolbar/status region inside the browser viewport, uses direct CAD-style viewport interaction (click-select, left-to-right window selection, right-to-left crossing selection, wheel zoom, middle-mouse pan, middle-double-click zoom extents), and now treats viewport clicks plus `Enter` / `Escape` as the primary command flow instead of visible snap/submit helper buttons, while persisting its Survey CAD project snapshot through WebNet project files, manifest bundles, and local recovery when the underlying source-signature still matches, without yet installing GPL-touched DWG viewer packages into core WebNet
