...
- TS sideshot post-adjust coordinates now stay on the solved projected/grid azimuth basis for setup- and target-derived bearings, avoiding the extra convergence rotation that had been throwing the combined-case `Chimney` and `Meridian` sideshots off by meters
