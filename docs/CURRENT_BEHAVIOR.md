...
- the top toolbar now includes a `Survey CAD` launcher beside `Project Options`; today it opens a branch-local renderer/model spike workspace that builds native CAD entities from current WebNet input or solved results, renders them through an internal SVG proof adapter, exposes native selection plus transaction-history seams (`Select All`, `Clear Selection`, `ERASE`, `Undo`, `Redo`), adds internal point-node / endpoint / midpoint / intersection / nearest snap feedback over the preview surface, supports typed/snap-driven `POINT`, `COGO PT`, `LINE`, `PLINE`, `INVERSE`, `MOVE`, `COPY`, and selection-driven `INTX` workflows with coordinate, azimuth-distance, and survey bearing-distance entry plus inverse bearing readout, and now persists its Survey CAD project snapshot through WebNet project files, manifest bundles, and local recovery when the underlying source-signature still matches, without yet installing GPL-touched DWG viewer packages into core WebNet
