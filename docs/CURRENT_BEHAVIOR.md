...
- the classic traverse adjusted-coordinate display transform now uses project-average grid/elevation factors from the traverse network while keeping the fixed point as the anchor, which tightens the remaining horizontal distance/coordinate drift in the parity listing
- the tiny residual classic traverse display scale/rotation no longer lives as a naked listing constant; it is now derived from an explicit legacy NB83 display factor/convergence contract while leaving the raw solve and raw traverse sections on the active parity CRS
- industry listings no longer include the WebNet-only `Observation Weighting Traceability` block; grid geodetic summaries now print fixed `DDD-MM-SS.SSSSSS` rows, honor the positive-west longitude display option, and the classic traverse NB83 geodetic rows invert the displayed coordinates through the CSRS double-stereographic contract used by the stored reference, while 3D station precision sections include the vertical column and the classic traverse bearing-distance block keeps the ground-distance continuation value aligned under the grid-distance column
