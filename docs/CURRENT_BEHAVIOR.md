...
- the top toolbar now includes a `Survey CAD` launcher beside `Project Options`; today it opens a dedicated full-page Survey CAD workspace that hides the input editor/results chrome while active, builds native CAD entities from current WebNet input or solved results, renders them through an internal SVG viewport, exposes native selection plus transaction-history seams (`Select All`, `Clear Selection`, `ERASE`, `Undo`, `Redo`), adds internal point-node / endpoint / midpoint / intersection / nearest snap feedback over the preview surface, supports typed/snap-driven `POINT`, `COGO PT`, `LINE`, `PLINE`, `ARC 3PT`, `TAN CURVE`, `INVERSE`, `MOVE`, `COPY`, and selection-driven `INTX` workflows with coordinate, azimuth-distance, survey bearing-distance, and tangent-curve radius entry plus inverse bearing readout, and now adds first viewport navigation controls (`Zoom In`, `Zoom Out`, `Zoom Extents`, wheel zoom, middle-mouse pan, drag-box selection) while persisting its Survey CAD project snapshot through WebNet project files, manifest bundles, and local recovery when the underlying source-signature still matches, without yet installing GPL-touched DWG viewer packages into core WebNet
...
- a dedicated `Survey CAD` workspace page that can be opened before any solve; it currently acts as the durable entry seam for CAD architecture docs plus the live native-model preview, persisted CAD workspace snapshot workflow, selection/undo command-history workflow, snapping-status workflow, typed command-input workflow (`POINT`, `COGO PT`, `LINE`, `PLINE`, `ARC 3PT`, `TAN CURVE`, `INVERSE`, `MOVE`, `COPY`, `INTX`), first viewport controls (`Zoom In`, `Zoom Out`, `Zoom Extents`, wheel zoom, middle-mouse pan, drag-box selection), and adapter-inspection/runtime-proof workflow
