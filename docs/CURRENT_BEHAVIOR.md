...
- preanalysis recommendations now also emit advisory-only path actions alongside applyable additive scenarios: bypassing weak intermediate hops with direct ties, decommissioning removable free intermediates once a bypass exists, and moving synthetic geometry toward the active bottleneck corridor; advisory rows remain visible in the report but are not auto-applied by the threshold planner
