...
- the top toolbar now includes a `Survey CAD` launcher beside `Project Options`; today it opens a branch-local renderer/model spike workspace that builds native CAD entities from current WebNet input or solved results, renders them through an internal SVG proof adapter, exposes native selection plus transaction-history seams (`Select All`, `Clear Selection`, `ERASE`, `Undo`, `Redo`), adds internal point-node / endpoint / midpoint / intersection / nearest snap feedback over the preview surface, supports typed/snap-driven `POINT`, `COGO PT`, `LINE`, `PLINE`, `ARC 3PT`, `TAN CURVE`, `INVERSE`, `MOVE`, `COPY`, and selection-driven `INTX` workflows with coordinate, azimuth-distance, survey bearing-distance, and tangent-curve radius entry plus inverse bearing readout, and now persists its Survey CAD project snapshot through WebNet project files, manifest bundles, and local recovery when the underlying source-signature still matches, without yet installing GPL-touched DWG viewer packages into core WebNet
...
- a `Survey CAD` workspace tab that can be opened before any solve; it currently acts as the durable entry seam for CAD architecture docs plus the live native-model/renderer spike preview, persisted CAD workspace snapshot workflow, selection/undo command-history spike, snapping-status workflow, typed command-input workflow (`POINT`, `COGO PT`, `LINE`, `PLINE`, `ARC 3PT`, `TAN CURVE`, `INVERSE`, `MOVE`, `COPY`, `INTX`), and adapter-inspection/runtime-proof workflow
