...
- active middle-drag 2D panning now freezes the derived viewport/culling basis at drag start while the live transform keeps moving, which reduces the visible overlay popping/jitter that came from SVG and other 2D layers re-deriving visible geometry mid-pan
