...
- preanalysis recommendations now also emit advisory-only path actions alongside applyable additive scenarios: bypassing weak intermediate hops with direct ties, decommissioning only removable free intermediates that stay connected once the bypass is added, and moving synthetic geometry toward the active bottleneck corridor; advisory rows remain visible in the report but are not auto-applied by the threshold planner
...
- the preanalysis threshold planner now remains additive-only even though the recommendation table can include advisory rows; when additive paths are exhausted it reports that only manual advisory network changes remain instead of trying to auto-apply them
