...
- active middle-drag 2D panning now previews motion by translating the rendered WebGL/canvas/SVG layers together as one slab while the drag-start derived viewport/culling basis stays frozen, and it commits the actual pan once the drag ends; this keeps survey geometry visually planted while the camera plane moves smoothly across the map
