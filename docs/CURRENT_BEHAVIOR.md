...
- the 2D OSM basemap now renders on the canvas background with per-tile mesh warping sampled in Web Mercator space, adaptive mesh density by on-screen tile size, and seam-suppression overdraw at shared tile/cell edges, while stations, lines, labels, polygons, and planning overlays remain on the existing SVG/tool layers
