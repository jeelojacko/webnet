...
- GNSS-only connected relative-confidence rows now use a narrow display-only blend for weakly anchored single-vector free/free pairs: when the direct GNSS vector covariance sits only modestly above the propagated pair covariance, the listing blends toward that direct covariance, and when the direct vector is much weaker but the endpoint-independent fallback is only modestly looser, the listing blends toward that fallback instead; this moves the remaining `GPS2-*` bearing-distance and relative-ellipse rows closer to the stored reference without changing solver coordinates or station sigmas
