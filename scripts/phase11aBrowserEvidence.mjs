// Phase 11A browser/worker evidence (EVIDENCE-ONLY, manual, never CI).
//
...
//
// Inputs: artifacts/evidence/phase11a-browser/requests.json (dumped by the
// Phase 11A Node evidence campaign from the same deterministic generator).
// Outputs: raw JSON under artifacts/evidence/phase11a-browser/
// (gitignored) + summary JSON at /tmp/phase11a-browser-summary.json.
...
const OUT_DIR = path.join(ROOT, 'artifacts/evidence/phase11a-browser');
const REQUESTS_PATH = path.join(OUT_DIR, 'requests.json');
const SUMMARY_PATH = '/tmp/phase11a-browser-summary.json';
const PORT = 4193;
...
const startServer = (nativeBundlePath) =>
  new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      ...
      if (url.pathname === '/nativeWorker.phase11a.js') {
    ...
    async ({ workerUrl: url, payload, total, timeoutMs }) => {
      ...
      try {
        ...
        for (let i = 0; i < total; i += 1) {
          ...
          const { wall, message } = await send(`phase11a-${i}`, keepFull);
...
const measureBrowser = async (tag, browser, fixtures, tsWorkerUrl, nativeWorkerUrl) => {
  ...
  // UI-responsiveness smoke after the heaviest fixture: trivial main-thread
  // round-trip must stay interactive (ms-scale) with big results in flight.
  const uiStart = Date.now();
  const uiState = await page.evaluate(() => ({ readyState: document.readyState }));
  const uiProbeMs = Date.now() - uiStart;
  await page.close();
  return { version, entries, uiProbeMs, uiReadyState: uiState.readyState };
};

const main = async () => {
  fs.mkdirSync(OUT_DIR, { recursive: true });
  const fixtures = JSON.parse(fs.readFileSync(REQUESTS_PATH, 'utf8'));
  if (fixtures.length !== 4) throw new Error(`Expected 4 dumped fixtures, found ${fixtures.length}.`);

  const harnessSrc = path.join(OUT_DIR, 'nativeWorker.phase11a.ts');
  const harnessBundle = path.join(OUT_DIR, 'nativeWorker.phase11a.js');
  ...
  const nativeWorkerUrl = `http://127.0.0.1:${PORT}/nativeWorker.phase11a.js`;
  ...
  try {
    ...
    try {
      const { version, entries, uiProbeMs, uiReadyState } = await measureBrowser('chromium', chromiumBrowser, fixtures, tsWorkerUrl, nativeWorkerUrl);
      raw.browsers.chromium = { version, uiProbeMs, uiReadyState };
    ...
    for (const attempt of [
      ...
      try {
        ...
        try {
          const { version, entries, uiProbeMs, uiReadyState } = await measureBrowser('firefox', fb, fixtures, tsWorkerUrl, nativeWorkerUrl);
          raw.browsers.firefox = { version, uiProbeMs, uiReadyState };
          raw.fixtures.push(...entries);
        } finally {
          await fb.close();
        }
    ...
    for (const entry of raw.fixtures) {
      ...
      // Transfer split (measurable part): engine-internal vs worker-messaging
      // vs main-thread accept, from pageWall / workerWall / engineElapsed.
      const nativeRuns = entry.native.measured;
      const workerOverEngine = nativeRuns.map((r) => (r.workerWallMs ?? Number.NaN) - (r.engineElapsedMs ?? Number.NaN)).filter((v) => Number.isFinite(v));
      const pageOverWorker = nativeRuns.map((r) => r.pageWallMs - (r.workerWallMs ?? Number.NaN)).filter((v) => Number.isFinite(v));
      summaryFixtures.push({
        ...
        transferSplitMs: {
          workerOverEngineMedian: median(workerOverEngine),
          pageOverWorkerMedian: median(pageOverWorker),
        },
    ...
    fs.writeFileSync(path.join(OUT_DIR, 'phase11a-browser-raw.json'), `${JSON.stringify(raw, null, 2)}\n`);
    ...
    console.log(`raw -> ${OUT_DIR}/phase11a-browser-raw.json`);
...
main().catch((error) => {
  console.error(`phase11a browser evidence FAILED: ${error instanceof Error ? error.stack : String(error)}`);
  process.exit(1);
});
