...
import type { AiStudyMapJob, AiStudyMapProposal, AiStudyMapResult } from '../src/ai/studyAiTypes';
import type {
  NbLawContentPackage,
...
import { hashText } from '../src/ai/studyAiUnitJobPrep';
import {
  FROZEN_UNIT_PREFLIGHT_RUN_ID,
  runFrozenUnitPreflight,
} from '../src/ai/studyAiUnitPreflight';
import {
...
describe.skipIf(!realDataPresent)('frozen canonical run preflight (real data)', () => {
  it('builds 4,251 jobs with the frozen priority distribution and a clean validation', { timeout: 300000 }, () => {
    ...
    expect(
      existsSync(
        studyTestContentPath( 'ai', 'runs', FROZEN_UNIT_PREFLIGHT_RUN_ID, 'run.json'),
      ),
    ).toBe(true);
  });
});
