...
import { STUDY_TEST_REPO_ROOT, studyTestContentPath } from './study_test_paths';
...
} from '../src/ai/studyAiValidation';
import type {
  AiStoredUnitProposal,
  AiStudyMapJob,
  AiStudyMapResult,
} from '../src/ai/studyAiTypes';
...
import type { ImportedLegalComponent } from '../src/studyTypes';
import type {
  NbLawContentPackage,
...
describe('AI authoring schemas and validation', () => {
  ...
  it('keeps v3 prompt specs focused on critical hardening requirements', () => {
    expect(existsSync(studyTestContentPath('ai', 'specs', 'study-map-v3.md'))).toBe(true);
    expect(existsSync(studyTestContentPath('ai', 'specs', 'unit-authoring-v3.md'))).toBe(true);
    expect(existsSync(studyTestContentPath('ai', 'specs', 'unit-authoring-v4.md'))).toBe(true);
    const mapSpec = readFileSync(studyTestContentPath('ai', 'specs', 'study-map-v3.md'), 'utf8');
    const unitSpec = readFileSync(studyTestContentPath('ai', 'specs', 'unit-authoring-v3.md'), 'utf8');
    const unitV4Spec = readFileSync(studyTestContentPath('ai', 'specs', 'unit-authoring-v4.md'), 'utf8');
