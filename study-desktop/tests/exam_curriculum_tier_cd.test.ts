...
} from '../src/examCurriculum/examCurriculumBuild';
import { EXAM_CURRICULUM_TOTAL, examCurriculumAllSpecs } from '../src/examCurriculum/examCurriculumCatalogAll';
import {
...
} from '../src/examCurriculum/examCurriculumCatalogTierCD';
import { buildExamCurriculumCorpusView } from '../src/examCurriculum/examCurriculumResolve';
import type { ExamCurriculumUnit } from '../src/examCurriculum/examCurriculumTypes';
...
describe('exam curriculum Tier-C/D catalog shape', () => {
  ...
  it('combines into exactly 157 cumulative specs in A→B→C→D→NAV→DRILL order with unique ids', () => {
...
describe('exam curriculum cumulative A+B+C+D build against the authoritative corpus', () => {
  it('resolves 157 units: 51 A / 43 B / 21 C / 6 D / 12 Navigation / 24 DRILL; 27 document_orientation and 0 core_concept added', () => {
    ...
    expect(manifest.units.filter((u) => u.tier === 'DRILL' && u.unitType === 'lookup_drill')).toHaveLength(24);
  });
