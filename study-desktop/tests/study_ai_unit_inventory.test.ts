...
import type { AiStudyMapResult } from '../src/ai/studyAiTypes';
import { FROZEN_MAP_RUN_ID, FROZEN_RESULT_ROW_COUNT } from '../src/ai/studyAiMapFreezeGate';
import { FREEZE_REPORT_REL_PATH } from '../src/ai/studyAiMapFreezeGate';
import {
...
} from '../src/ai/studyAiUnitInventory';
import {
...
const realRunDir = studyTestContentPath('ai', 'runs');
...
describe.skipIf(!realRunPresent)('frozen canonical run inventory (real data)', () => {
  it('builds the inventory with the brief-guaranteed counts', () => {
    const inventory = buildFrozenMapGroupInventory({
      runDir: join(realRunDir, FROZEN_MAP_RUN_ID),
      corpusPackagePath: studyTestContentPath(
        'packages',
        'nb-sit-statute-corpus.content-package.json',
      ),
      freezeReportPath: join(STUDY_TEST_REPO_ROOT, FREEZE_REPORT_REL_PATH),
    });
