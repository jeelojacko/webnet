...
const RUNS = studyTestContentPath( 'ai', 'runs');
...
describe('Study Map local comparison CLI planning', () => {
  ...
  it('resolves V1 known-good locations against the V1 run directory deterministically', () => {
    ...
    expect(__studyAiLocalMapCompareTest.resolveV1Location(`${V1_RUN}/results/batch-001.results.jsonl`, V1_RUN)).toBe(studyTestContentPath('ai', 'runs', V1_RUN, 'results', 'batch-001.results.jsonl'));
    expect(__studyAiLocalMapCompareTest.resolveV1Location('results/batch-001.results.jsonl', V1_RUN)).toBe(studyTestContentPath('ai', 'runs', V1_RUN, 'results', 'batch-001.results.jsonl'));
    expect(__studyAiLocalMapCompareTest.resolveV1Location('./results/batch-001.results.jsonl', V1_RUN)).toBe(studyTestContentPath('ai', 'runs', V1_RUN, 'results', 'batch-001.results.jsonl'));
  });
});
