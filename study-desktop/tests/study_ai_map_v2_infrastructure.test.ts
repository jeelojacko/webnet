...
import { validateAiStudyMapResult } from '../src/ai/studyAiValidation';
import { STUDY_MAP_V3_RESULT_SCHEMA } from '../src/ai/studyAiResultContract';
import { authoringInputFingerprint } from '../../scripts/studyAiFingerprint';
import type { AiStudyMapJob, AiStudyMapResult } from '../src/ai/studyAiTypes';
import type {
  NbLawContentPackage,
  NbLawDocumentComponent,
} from '../src/content/nbLawTypes';

const testRuns = ['ai-test-local-map-runner'];

afterEach(() => {
  testRuns.forEach((runId) =>
    rmSync(studyTestContentPath( 'ai', 'runs', runId), { recursive: true, force: true }),
  );
});

const readCorpusPackage = (): NbLawContentPackage =>
  JSON.parse(
    readFileSync(studyTestContentPath('packages', 'nb-sit-statute-corpus.content-package.json'), 'utf8'),
...
const writeJobRun = (job: AiStudyMapJob): void => {
  const runDir = studyTestContentPath( 'ai', 'runs', job.runId);
...
const runDirFor = (job: AiStudyMapJob): string =>
  studyTestContentPath( 'ai', 'runs', job.runId);
...
const readProviderEvents = (runId: string): Array<Record<string, unknown>> => {
  const path = studyTestContentPath( 'ai', 'runs', runId, 'reports', 'provider-events.jsonl');
...
describe('Study Map V2 infrastructure repairs', () => {
  ...
  it('loads the canonical Study Map V3 spec and fails closed when missing', () => {
    expect(loadStudyMapV3Spec(studyTestContentPath('ai', 'specs', 'study-map-v3.md'))).toContain('curriculum mapping, not legal analysis');
    expect(() => loadStudyMapV3Spec(studyTestContentPath('ai', 'specs', 'does-not-exist.md'))).toThrow(
      /spec not found/,
    );
  });

  it('guards historic-applicability source discipline in the Study Map V3 spec', () => {
    const spec = loadStudyMapV3Spec(studyTestContentPath('ai', 'specs', 'study-map-v3.md'));
...
it('keeps exact statutory wording in the regression corpus anchors', () => {
  ...
  it('sends the canonical Study Map V3 spec in the local runner system prompt', async () => {
    ...
    expect(system.length).toBeGreaterThan(loadStudyMapV3Spec(studyTestContentPath('ai', 'specs', 'study-map-v3.md')).length);
  });
  ...
  it('retries a missing suggestedPriority against the previous invalid response and accepts the corrected result', async () => {
    ...
    const line = readFileSync(
      studyTestContentPath( 'ai', 'runs', job.runId, 'results', 'local-map.results.jsonl'),
      'utf8',
    ).trim();
  ...
  it('sends optional reasoning effort without changing strict JSON Schema response format', async () => {
    ...
    rmSync(studyTestContentPath( 'ai', 'runs', job.runId), { recursive: true, force: true });
    writeJobRun(job);
  ...
  it('records resolved inference config provenance for accepted and rejected attempts', async () => {
    ...
    const accepted = JSON.parse(
      readFileSync(
        studyTestContentPath( 'ai', 'runs', job.runId, 'results', `${job.jobId}.provenance.json`),
        'utf8',
      ),
    );
  ...
  it('skips already accepted local results on resume without duplicate lines', async () => {
    const job = jobFixture();
    writeJobRun(job);
    mkdirSync(studyTestContentPath( 'ai', 'runs', job.runId, 'results'), { recursive: true });
    writeFileSync(
      studyTestContentPath( 'ai', 'runs', job.runId, 'results', 'local-map.results.jsonl'),
