...
import type { BuildImportConflictSummaryArgs, ImportConflict } from './importConflictReview';
...
export const parseExistingInput = ({
  currentInput,
  currentIncludeFiles,
  parseSettings,
  projectInstruments,
...
export const buildCurrentInputLines = (input: string): string[] => input.split(/\r?\n/);
...
export const buildImportedObservationConflictKey = (
...
export const buildExistingObservationCounts = (parsed: ParseResult | null): Map<string, number> => {
...
const addStationDetailConflicts = ({
  conflicts,
  currentInput,
  existingDescription,
  existingStation,
  ...
  existingStation: StationMap[string];
  index: number;
  parseSettings: ParseSettings;
  station: ImportedControlStationRecord;
  stationId: string;
  ...
  const common = {
    ...
    sourceLine: station.sourceLine,
    existingSourceLines: findExistingControlSourceLines(currentInput, station.stationId),
    relatedItems: [{ kind: 'control' as const, index }],
  };
  const existingSummary = formatExistingStationSummary(
    station.stationId,
    existingStation,
    existingDescription,
  );
  const incomingSummary = formatIncomingStationSummary(station);
