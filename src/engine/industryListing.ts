...
export const buildIndustryStyleListingText = (
  ...
  runDiagnostics: IndustryListingRunDiagnostics,
    ...
    const renderTextTable = (
      ...
      lines.push(
        headers
          .map((h, col) => formatCell(h, widths[col], right.has(col)))
          .join('  '),
      );
      rows.forEach((row) => {
        lines.push(
          headers
            .map((_, col) => formatCell(row[col] ?? '', widths[col], right.has(col)))
            .join('  '),
        );
      });
    };
    ...
    const relationshipRows = relationshipPairs
      .map((pair) => {
        ...
        const sigmaDist95 = rel?.sigmaDist != null ? (rel.sigmaDist * unitScale * 1.96).toFixed(4) : '-';
        const ppm95 =
          rel?.sigmaDist != null && distance !== '-'
            ? ((rel.sigmaDist * 1.96 * 1_000_000) / Math.max(1e-12, Math.abs(Number(distance) / unitScale))).toFixed(4)
            : '-';
        return {
          from,
          to,
