/**
 * Phase 8A.6 packed sparse normal accumulator + covariance sentinel (TEST/EVIDENCE ONLY).
 *
 * Pure test-only sentinel built from the same packed sparse inputs the
 * production sparse backend consumes. It never allocates a dense weight
 * matrix P and never reconstructs a full dense Qxx: the normal matrix N
 * (n x n, n <= 128) is accumulated directly from packed design rows plus
 * packed upper-triangle weights, then only the probed covariance columns
 * are solved via the existing scaling/factor/solve primitives
 * (`scaleNormalMatrix`, `choleskyDecomposeWithDamping`,
 * `solveSPDFromCholesky`). Nothing in production imports this module; no
 * routing, tolerance, or engine behavior changes.
 *
 * Designs:
...
export const PHASE8A6_SENTINEL_MAX_UNKNOWN_COUNT = 128;

/** Relative tolerance for C1 dense-selected agreement at selected entries. */
export const PHASE8A6_C1_RELATIVE_TOLERANCE = 1e-6;

/** Absolute floor under the C1 relative tolerance (mirrors contract gates). */
export const PHASE8A6_C1_ABSOLUTE_FLOOR = 1e-12;

/** Infinity-norm tolerance for the C2 inverse residual per column. */
export const PHASE8A6_C2_RESIDUAL_TOLERANCE = 1e-6;

/** Symmetry tolerance for probed (r,c)/(c,r) pairs. */
export const PHASE8A6_SYMMETRY_TOLERANCE = 1e-9;

export interface Phase8a6PackedDesign {
  rowOffsets: Int32Array;
  columns: Int32Array;
  values: Float64Array;
}

export interface Phase8a6PackedUpperWeights {
  rows: Int32Array;
  columns: Int32Array;
  values: Float64Array;
}

export interface Phase8a6PackedSystem {
  design: Phase8a6PackedDesign;
  weights: Phase8a6PackedUpperWeights;
  observationEquationCount: number;
  parameterCount: number;
}

export interface Phase8a6ProbeResult {
 ...
 */
export const accumulatePackedNormal = (system: Phase8a6PackedSystem): Matrix => {
  ...
  if (n > PHASE8A6_SENTINEL_MAX_UNKNOWN_COUNT) {
    throw new Error(
      `sentinel cap: ${n} unknowns exceed ${PHASE8A6_SENTINEL_MAX_UNKNOWN_COUNT} (fail-closed).`,
    );
  }
...
export const probeSelectedCovariance = (
  ...
  if (n === 0 || n > PHASE8A6_SENTINEL_MAX_UNKNOWN_COUNT) {
...
export interface Phase8a6C1Result {
  pass: boolean;
  reasons: string[];
...
export const evaluateSentinelC1 = (
  probed: number[],
  reference: number[],
  tolerance = PHASE8A6_C1_RELATIVE_TOLERANCE,
): Phase8a6C1Result => {
  ...
  probed.forEach((value, k) => {
    ...
    const allowed = Math.max(PHASE8A6_C1_ABSOLUTE_FLOOR, tolerance * Math.max(1, Math.abs(expected)));
...
export interface Phase8a6C2Result {
  pass: boolean;
  reasons: string[];
  maxResidual: number;
...
export const evaluateSentinelC2 = (
  ...
  tolerance = PHASE8A6_C2_RESIDUAL_TOLERANCE,
): Phase8a6C2Result => {
  const reasons: string[] = [];
  const failClosed = (reason: string): Phase8a6C2Result => ({
    pass: false,
    reasons: [reason],
...
export interface Phase8a6C3Result {
  pass: boolean;
  reasons: string[];
}
...
export const validateSentinelPhysical = (args: {
  ...
  args.values.forEach((value, k) => {
    ...
    const allowed = Math.max(PHASE8A6_SYMMETRY_TOLERANCE, 1e-9 * Math.max(1, Math.abs(value)));
