...
import { buildLoopGraphTraversal, type LoopGraphEdgeBase } from './adjustmentLoopGraph';
...
export const buildGpsLoopDiagnostics = ({
  ...
  type LoopEdge = LoopGraphEdgeBase & {
    obsId: number;
    dE: number;
    dN: number;
    distance: number;
  ...
  const { treeEdgeIdx, buildPath } = buildLoopGraphTraversal(edges);
