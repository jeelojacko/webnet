...
const commentLine = (line: string): string => {
  ...
  if (isCommentedLine(line)) return line;
...
export const toggleHashCommentsInSelection = (
  ...
  const shouldUncomment = hasContent && lines.every(isCommentedLine);
  const toggledLines = shouldUncomment ? lines.map(uncommentLine) : lines.map(commentLine);
  const toggledBlock = toggledLines.join('\n');

  if (toggledBlock === block) {
    return { text, selectionStart: lineStart, selectionEnd: lineEnd, changed: false };
  }
