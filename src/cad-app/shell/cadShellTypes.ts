...
export interface CadWorkspaceSnapshot {
  ...
  /**
   * Phase 18U — analysis maps + legends (derived band status/quantities;
   * definitions live on the project). Optional: a workspace without analysis
   * wiring leaves it undefined and analysis UI hides (never fake data).
   */
  analysis?: CadAnalysisSnapshot | null;
...
export interface CadShellActions {
  ...
  /** Phase 18U — select an analysis map (Toolspace/manager/Properties converge). */
  selectAnalysis?: (_analysisId: string | null) => void;
  /** Phase 18U — select an analysis legend. */
  selectAnalysisLegend?: (_legendId: string | null) => void;
  /**
   * Phase 18U — create a map for the current selection (source + equal bands
   * are generated by the workspace; returns display text for the notice seam).
   */
  createAnalysis?: (_kind: 'elevation' | 'slope-percent' | 'signed-depth') => string;
  /** Phase 18U — manual analysis Calculate (explicit action only). */
  requestAnalysis?: (_analysisId: string) => string;
  /** Phase 18U — manual Calculate for the selected map (ribbon/registry path). */
  calculateSelectedAnalysis?: () => void;
  /** Phase 18U — arm a one-shot viewport pick for analysis inquiry (null disarms). */
  startAnalysisPick?: (_analysisId: string | null) => void;
  /** Phase 18U — E/N inputs resolve to metric + band display text (pure read). */
  queryAnalysis?: (_analysisId: string, _x: number, _y: number) => string | null;
  /**
