...
import { normalizePath } from './legacyCorpusHarnessSupport';
...
export const runProjectChecks = (project: LegacyCorpusProject): ProjectCheckResult => {
  ...
  const result = new LSAEngine({ input, maxIterations: 15, parseOptions }).solve();
  ...
  const missingRequiredDiagnostics = (project.expected.requiredDiagnostics ?? []).filter(
    (code) => !runModeDiagnosticCodes.includes(code),
  );
  const failures = buildProjectFailures({
    project,
    parseSuccess,
    runSuccess,
