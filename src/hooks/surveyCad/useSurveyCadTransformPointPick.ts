...
const handleHelmert2DPick = (options: TransformPointPickOptions): boolean => {
  const { current, point, replaceSession } = options;
  if (current.key !== 'HELMERT2D') return false;
  // Explicit pairs only: alternating source pick -> target pick completes
  // one pair. No name-matching anywhere; typed `sx,sy,tx,ty` pairs land in
  // the typed submit path, never here.
...
const handleGridGroundPick = (options: TransformPointPickOptions): boolean => {
  const { current, point, replaceSession } = options;
  if (current.key !== 'GRIDGROUND') return false;
  if (!current.origin) {
    replaceSession({ ...current, origin: point, inputValue: '', resultText: undefined });
    return true;
  }
  // Origin captured; the factor is typed. Swallow further picks so a stray
  // click cannot re-anchor mid-command.
  replaceSession({
    ...current,
    inputValue: '',
    resultText: 'GRIDGROUND origin captured. Type a positive combined scale factor and press Enter.',
  });
  return true;
};
