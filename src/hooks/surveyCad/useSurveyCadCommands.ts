...
const traverseInputHasExplicitLabel = (token: string): boolean => {
  const { label } = splitLabelFromBody(token);
  return label != null && label.length > 0;
};

const isTraverseAutoGeneratedPointLabel = (point: CommandPoint): boolean =>
  point.label === `${point.x.toFixed(3)},${point.y.toFixed(3)}` || /^tp\d+$/i.test(point.label);

const nextTraverseAutoPointOrdinal = (points: readonly CommandPoint[]): number => {
  let maxOrdinal = 0;
  points.forEach((point) => {
    const match = /^tp(\d+)$/i.exec(point.label);
    if (!match) return;
...
const normalizeTraverseDraftPoint = (
  point: CommandPoint,
  ...
  const shouldAutoLabel =
    options?.rawInput != null
      ? !traverseInputHasExplicitLabel(options.rawInput)
      : isTraverseAutoGeneratedPointLabel(point);
  if (!shouldAutoLabel) return point;
  return {
    ...point,
    label: `tp${nextTraverseAutoPointOrdinal(existingPoints)}`,
  };
};
...
const parseInputPoint = (inputValue: string, basePoint: CommandPoint | null): CommandPoint | null =>
    ...
    if (current.key === 'PLINE' || current.key === 'TRAVERSE') {
      const traversePoint =
        current.key === 'TRAVERSE'
          ? normalizeTraverseDraftPoint(point, current.inputPoints)
          : point;
      replaceSession(
        current.key === 'TRAVERSE'
          ? {
              ...current,
              points: [...current.inputPoints, traversePoint],
              inputPoints: [...current.inputPoints, traversePoint],
              legInputs:
  ...
  const rebuildTraverseDraftFromLegInputs = (
    ...
    for (const legInput of legInputs) {
      ...
      nextPoints.push(
        normalizeTraverseDraftPoint(parsedPoint, nextPoints, {
          rawInput: legInput,
        }),
      );
    }
  ...
  const submitSessionInput = () => {
    ...
    if (session.key === 'TRAVERSE') {
      ...
      const normalizedPoint = normalizeTraverseDraftPoint(parsedPoint, session.inputPoints, {
        rawInput,
      });
      replaceSession({
        ...session,
  ...
  const appendTraverseDraftPoint = (inputValue: string) => {
    ...
    const normalizedPoint = normalizeTraverseDraftPoint(parsedPoint, current.inputPoints, {
      rawInput: inputValue,
    });
    replaceSession({
      ...current,
