...
describe('CLI phase 2 output modes', () => {
  ...
  it('accepts EPSG aliases for --crs-id and normalizes to catalog ids', () => {
    ...
    const usNvCentral = runCli([
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_NV_CENTRAL',
    ]);
    expect(usNvCentral.status).toBe(0);
    const usNvCentralPayload = JSON.parse(usNvCentral.stdout);
    expect(usNvCentralPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usNvCentralPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_NV_CENTRAL');

    const usNmWestFt = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usNmWestFt.status).toBe(0);
    const usNmWestFtPayload = JSON.parse(usNmWestFt.stdout);
    expect(usNmWestFtPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usNmWestFtPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_NM_WEST_FTUS');
    ...
    const usNh = runCli([
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_NH',
    ]);
    expect(usNh.status).toBe(0);
    const usNhPayload = JSON.parse(usNh.stdout);
    expect(usNhPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usNhPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_NH');

    const usOhSouthFt = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usOhSouthFt.status).toBe(0);
    ...
    const usArNorth = runCli([
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_AR_NORTH',
    ]);
    expect(usArNorth.status).toBe(0);
    const usArNorthPayload = JSON.parse(usArNorth.stdout);
    expect(usArNorthPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usArNorthPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_AR_NORTH');

    const usOkSouthFt = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usOkSouthFt.status).toBe(0);
    const usOkSouthFtPayload = JSON.parse(usOkSouthFt.stdout);
    expect(usOkSouthFtPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usOkSouthFtPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_OK_SOUTH_FTUS');

    const usAkZone1 = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_AK_ZONE_1',
    ]);
    expect(usAkZone1.status).toBe(0);
    const usAkZone1Payload = JSON.parse(usAkZone1.stdout);
    expect(usAkZone1Payload.parseState?.coordSystemMode).toBe('grid');
    expect(usAkZone1Payload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_AK_ZONE_1');

    const usAkZone10 = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usAkZone10.status).toBe(0);
    const usAkZone10Payload = JSON.parse(usAkZone10.stdout);
    expect(usAkZone10Payload.parseState?.coordSystemMode).toBe('grid');
    expect(usAkZone10Payload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_AK_ZONE_10');

    const usMiCentral = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_MI_CENTRAL',
    ]);
    expect(usMiCentral.status).toBe(0);
    const usMiCentralPayload = JSON.parse(usMiCentral.stdout);
    expect(usMiCentralPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usMiCentralPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_MI_CENTRAL');

    const usNdSouth = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usNdSouth.status).toBe(0);
    const usNdSouthPayload = JSON.parse(usNdSouth.stdout);
    expect(usNdSouthPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usNdSouthPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_ND_SOUTH');

    const usAzCentral = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_AZ_CENTRAL',
    ]);
    expect(usAzCentral.status).toBe(0);
    const usAzCentralPayload = JSON.parse(usAzCentral.stdout);
    expect(usAzCentralPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usAzCentralPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_AZ_CENTRAL');

    const usAzWest = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usAzWest.status).toBe(0);
    const usAzWestPayload = JSON.parse(usAzWest.stdout);
    expect(usAzWestPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usAzWestPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_AZ_WEST');

    const usOrNorth = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_OR_NORTH',
    ]);
    expect(usOrNorth.status).toBe(0);
    const usOrNorthPayload = JSON.parse(usOrNorth.stdout);
    expect(usOrNorthPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usOrNorthPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_OR_NORTH');

    const usOrSouthFt = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
    ...
    expect(usOrSouthFt.status).toBe(0);
    const usOrSouthFtPayload = JSON.parse(usOrSouthFt.stdout);
    expect(usOrSouthFtPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usOrSouthFtPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_OR_SOUTH_FTUS');

    const usNyLongIsland = runCli([
      '--input',
      STABLE_INPUT,
      '--output',
      'json',
      ...
      'grid',
      '--crs-id',
      'US_NAD83_2011_SPCS_NY_LONG_ISLAND',
    ]);
    expect(usNyLongIsland.status).toBe(0);
    const usNyLongIslandPayload = JSON.parse(usNyLongIsland.stdout);
    expect(usNyLongIslandPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usNyLongIslandPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_NY_LONG_ISLAND');
    ...
    expect(usScFt.status).toBe(0);
    const usScFtPayload = JSON.parse(usScFt.stdout);
    expect(usScFtPayload.parseState?.coordSystemMode).toBe('grid');
    expect(usScFtPayload.parseState?.crsId).toBe('US_NAD83_2011_SPCS_SC_FTUS');
  }, 120000);
