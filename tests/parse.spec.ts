...
describe('parseInput', () => {
  ...
  it('supports .CRS MODE/ID and .CRS GRID <id> aliases for coordinate-system selection', () => {
    ...
    const viaUsMdCanonical = parseInput(['.CRS GRID US_NAD83_2011_SPCS_MD', 'C A 0 0 0 ! !'].join('\n'));
    expect(viaUsMdCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMdCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_MD');

    const viaUsMaMainlandFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6492', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsMaMainlandFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMaMainlandFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_MA_MAINLAND_FTUS');

    const viaUsMnCentralCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_MN_CENTRAL', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsMnCentralCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMnCentralCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_MN_CENTRAL');

    const viaUsMnSouthFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6505', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsMnSouthFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMnSouthFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_MN_SOUTH_FTUS');

    const viaUsIlEastCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_IL_EAST', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsIlEastCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsIlEastCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_IL_EAST');

    const viaUsInWestFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6461', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsInWestFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsInWestFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_IN_WEST_FTUS');

    const viaUsMsTmCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_MS_TM', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsMsTmCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMsTmCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_MS_TM');

    const viaUsMoWestEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6513', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsMoWestEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMoWestEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_MO_WEST');

    const viaUsNvCentralCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_NV_CENTRAL', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsNvCentralCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsNvCentralCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_NV_CENTRAL');

    const viaUsNmWestFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6533', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsNmWestFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsNmWestFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_NM_WEST_FTUS');

    const viaUsNhCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_NH', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsNhCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsNhCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_NH');

    const viaUsOhSouthFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6551', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsOhSouthFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsOhSouthFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_OH_SOUTH_FTUS');

    const viaUsIdCentralCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_ID_CENTRAL', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsIdCentralCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsIdCentralCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_ID_CENTRAL');

    const viaUsIaSouthFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6465', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsIaSouthFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsIaSouthFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_IA_SOUTH_FTUS');

    const viaUsArNorthCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_AR_NORTH', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsArNorthCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsArNorthCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_AR_NORTH');

    const viaUsOkSouthFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6555', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsOkSouthFtEpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsOkSouthFtEpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_OK_SOUTH_FTUS');

    const viaUsAkZone1Canonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_AK_ZONE_1', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsAkZone1Canonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsAkZone1Canonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_AK_ZONE_1');

    const viaUsAkZone10EpsgAlias = parseInput(
      ['.CRS GRID EPSG:6403', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsAkZone10EpsgAlias.parseState.coordSystemMode).toBe('grid');
    expect(viaUsAkZone10EpsgAlias.parseState.crsId).toBe('US_NAD83_2011_SPCS_AK_ZONE_10');

    const viaUsMiCentralCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_MI_CENTRAL', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsMiCentralCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsMiCentralCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_MI_CENTRAL');

    const viaUsNdSouthEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6546', 'C A 0 0 0 ! !'].join('\n'),
    );
    ...
    const viaUsOrNorthCanonical = parseInput(
      ['.CRS GRID US_NAD83_2011_SPCS_OR_NORTH', 'C A 0 0 0 ! !'].join('\n'),
    );
    expect(viaUsOrNorthCanonical.parseState.coordSystemMode).toBe('grid');
    expect(viaUsOrNorthCanonical.parseState.crsId).toBe('US_NAD83_2011_SPCS_OR_NORTH');

    const viaUsOrSouthFtEpsgAlias = parseInput(
      ['.CRS GRID EPSG:6561', 'C A 0 0 0 ! !'].join('\n'),
    );
