...
- Current behavior also includes cluster-detection Phase 1 diagnostics: post-adjust cluster candidate detection with deterministic keys (`CL-<n>-<rep>`), linkage mode (`single` or `complete`), and coord-mode-aware tolerances (2D/3D), surfaced in report/processing summary/export outputs and run logs.
