...
- Current behavior also includes automatic sideshot detection Phase 1 diagnostics for non-redundant `M` records: post-adjust redundancy-aware candidate detection with fixed-control target safety exclusion, run-log summaries, report/processing-summary candidate tables, and export/listing sections for candidate traceability.
