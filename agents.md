...
- Current behavior also includes automatic sideshot detection Phases 1-2 for non-redundant `M` records: post-adjust redundancy-aware candidate detection with fixed-control target safety exclusion, run-log summaries, report/processing-summary candidate tables, and export/listing sections for candidate traceability, plus Project Options/parse-state toggle support (`Auto-Sideshot` UI control and `.AUTOSIDESHOT`/`/AUTOSIDESHOT` parser directives, default ON).
