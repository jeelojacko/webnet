...
- Current behavior also includes `.ORDER` station-triplet token support (`ATFROMTO` / `FROMATTO`), `.UNITS` angle-unit token support (`DMS` / `DD`), direction-set processing mode support (reduced-by-target or raw per-shot), run-pinned solve-profile diagnostics surfaced in report/export (profile, direction-set mode, stochastic/reduction switches, STAR fallback, angle centering model), and a-priori precision scaling fallback for zero/non-positive DOF so radial/non-redundant setups still report non-zero coordinate standard deviations.
