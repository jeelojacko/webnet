...
- [ ] Survey CAD alignment/stationing batch (June 2026): current slice: create native `ALIGN` from selected line/arc chains, add selection-driven `STA` station/offset reporting for `alignment + point`, add typed `STA PT` station-offset point creation from a selected alignment, persist alignment/station provenance, and keep shared alignment length/station/projection helpers ready for later station equations, interval points, and offset alignments.
