- [ ] Study Unit Authoring: local-Qwen calibration run (next milestone, NOT started): run sibling run `ai-units-2026-09-02-frozen-map-cal80-v4` (80 jobs, 10 batches of 8) through the `local-openai-compatible` local-Qwen provider per `reports/unit-calibration-80-launch-manifest-20260902.json`, ingest + validate results, review against the 80-row review pack, iterate on any generic Unit Authoring v4 spec/validator defects discovered, and — only if the prompt spec changes — regenerate the full-corpus preflight job set at the new prompt version. Stop before full-corpus unit generation. No model invocation occurred during the bridge/preparation batch.
