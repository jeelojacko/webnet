- [ ] Survey CAD parcel auto frontage-path batch (July 2026): replace the single-edge frontage-strip fallback with a depth-aware frontage-path auto solver that honors max-depth and remainder settings, supports one-edge, multi-segment corner frontage, and arc-length frontage, and lock it with focused engine/workspace fixtures before the full validation batch.
