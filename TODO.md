...
- [ ] add planned improvements to todo list in phased implementations before starting on imlementation
  ...
  - [ ] Phase 4 - industry-standard result parity
    ...
    - [ ] Batch 9 - GNSS-only parity startup rotation
      - [ ] Current scope: close the mixed combined-case statistical-summary drift by fixing default-weighted leveling rows that currently inherit inline TS instruments with no differential-level model, then re-diff the remaining GNSS summary/precision seams without disturbing the retained leveling-only and traverse parity locks
