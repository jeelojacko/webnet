- [ ] Study Phase 4B.2.1 V4 Unit Authoring remediation (August 2026): preserve `ai-units-4b2-expanded-s48-v4` artifacts, audit the deterministic Phase 4B.2 generation path, remove deterministic/template prose authoring from the production external-Codex path, harden Unit Authoring validation for generic questions, truncation, definition responsiveness, duplicate/nonresponsive answers, unsupported-reference false positives, and map-revision grounding, rerun the same 48 approved Map groups under a new V4-fix run ID without full-corpus generation, generate remediation comparison artifacts, update docs/tests, validate, commit, and stop for human review.
  ...
  - [ ] Complete genuine external per-job Codex authoring for `ai-units-4b21-expanded-s48-v4fix/results/*.jsonl`, then generate old-vs-remediated before/after comparison totals for human review. Do not use deterministic templates or start full-corpus generation.
