...
- [ ] add planned improvements to todo list in phased implementations before starting on imlementation
  ...
  - [ ] Phase 4 - industry-standard result parity
    ...
    - [ ] Batch 9 - GNSS-only parity startup rotation
      - [ ] Current scope: keep closing the last mixed combined-case summary drift after fixing the default-weighted leveling fallback and removing differential-level vertical-centering inflation; the remaining work is now a very small residual summary delta rather than a category-level vertical mismatch, and must preserve the retained leveling-only and traverse parity locks
      ...
      - [ ] Check whether the adjusted GNSS covariance-vector display contract should round station heights to the same precision shown in the listing before building local-topocentric display vectors
