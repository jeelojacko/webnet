...
- [ ] add planned improvements to todo list in phased implementations before starting on imlementation
  ...
  - [ ] Phase 4 - industry-standard result parity
    ...
    - [ ] Batch 9 - GNSS-only parity startup rotation
      - [ ] Current scope: keep closing the GNSS-only parity gap now that the missing vertical-deflection setting, covariance-vector dialect (`.GPS WEIGHT COVARIANCE`, `.GPS FACTOR`, `G0/G1/G2/G3`), and local-topocentric covariance-vector solve path are in place, while preserving the retained leveling and traverse locks; align the GNSS adjustment statistical summary with the industry equation-count contract (`GPS Deltas` reported as scalar components rather than raw vector rows) before chasing the remaining precision-section drift
