...
- Open `Survey CAD` from the toolbar to work in a standalone CAD drawing. Use the CAD file controls for `New Drawing`, `Open Drawing`, `Save Drawing`, `Export Drawing`, and `Import Adjusted Points`; Modify carries `Project Transform` (whole-drawing Helmert/Grid-Ground, see `docs/evidence/phase18r-project-transform-architecture.md`); the Surface ribbon Definition Tools group and Surface Manager compose two CURRENT surfaces into one explicit TIN (composite copy or paste in place); adjustment projects no longer auto-own CAD state. Surface composition behavior is inventoried in [docs/CURRENT_BEHAVIOR.md](docs/CURRENT_BEHAVIOR.md).
- Open `Study` from the toolbar, or visit `/study`, for a separate local-first New Brunswick statute and survey-law study app. Its Manage page can import the generated official NB law content package for offline legal reading, reset local Study data for testing, use worker-backed indexed Library search, source-linked study units with editable answer rubrics and preview mode, deterministic editable study-unit drafts, custom unlinked study units, provider-neutral AI authoring proposals under Authoring, and FSRS-backed due review sessions with non-mutating practice/preview paths, next-review empty-state timing, and persisted latest-rating undo. The sidebar **Exam Prep** item (Home `/study/exam-prep`, Learn `/study/learn`, Recall `/study/review`, Lookup Drills `/study/drills`; the legacy `/study/exam-curriculum` path renders Learn) runs the open-book exam workflow over the frozen 157-unit Exam Curriculum (51 Tier-A, 43 Tier-B, 21 Tier-C, 6 Tier-D, 12 Navigation, 24 lookup drills): Learn marks the 133 A-D/NAV units as studied (Home shows studied X/133, recall due, introduced X/57, and 57-card totals plus a Suggested study flow, a deterministic advisory Recommended Now next action, Today's activity derived on the local date, and five readiness dimensions incl. Recognition coverage X/317 + latest accuracy, Locate coverage X/452 + latest accuracy, and Lookup Drills exam-ready X/24), Recall runs explicit frozen review sessions over the 57 FSRS-scheduled cards derived from `mustRecall` (Start Recall Session freezes due + priority new cards once, expected answer hidden until Reveal, immutable attempts saved atomically, ratings never refill a session, then Start Another Session), and Recognition (`/study/recognition`) and Locate (`/study/locate`) run frozen 10-question retrieval sprints over exact cue and `mustLocate` pools, while Lookup Drills (`/study/drills`) record persistent self-assessed attempts with local-date readiness and statute-reader deep links. A provisional **Mock Exam** tab (`/study/mock-exam`) runs a timed simulation over the same frozen curriculum from a versioned provisional profile (currently 150 minutes, 30 mixed questions worth 42 self-assessed points, open-book with the built-in statute library, no official pass mark yet): deterministic same-seed papers, deadline-based timer with autosave that survives reload, submit confirmation, self-grading against frozen answers, a finalize gate, practice-score results (never pass/fail), abandon/resume/history, and a profile snapshot stored with every session — mock runs never update FSRS or ordinary practice attempts. Only the 57 Recall cards use FSRS; all other practice uses immutable attempt history. Progress, scheduling, attempts, session settings, and mock sessions remain in the schema-v10 Exam Prep stores (the four original stores plus `examPrepMockSessions`), bound to the frozen manifest content hash, and exportable/importable with the Manage page.
- For the Study official NB law pilot content pipeline, use `npm run study:fetch-nb-laws`, `npm run study:normalize-nb-laws`, and `npm run study:build-content-pack`. For the Phase 4A NB SIT statute corpus, use `npm run study:corpus:inventory` first; current fetch succeeds for confirmed `laws.gnb.ca` successors and ANBLS PDFs, while normalize/build intentionally block until the bilingual ANBLS PDFs have reliable English-only extraction selectors. Use `npm run study:audit-generation` to generate developer QA reports for deterministic source-linked study-unit generation. These development commands fetch only manifest-listed official documents when fetching is explicitly run and do not run during normal browser use. For the open-book Exam Curriculum manifest, use `npm run study:exam-curriculum:build`; it deterministically resolves the 157 curated units (51 Tier-A + 43 Tier-B + 21 Tier-C + 6 Tier-D + 12 Navigation + 24 DRILL lookup drills) against the authoritative NB SIT statute corpus package and writes `study-content/exam-curriculum/nb-sit-exam-curriculum-v1.json` plus dated reports under `reports/exam-curriculum/` (with per-tier breakdowns and Tier-A/B/C/D/NAV/DRILL projection hashes and source-coverage-by-document metrics), with no model inference. For a deterministic mechanical QA audit of the frozen Exam Prep task pools, run `npm run study:exam-prep:qa-report`; it writes `reports/exam-prep/recall-quality-audit.md` (all 57 recall cards) and `reports/exam-prep/recognition-quality-audit.md` (all 317 recognition cues) from the bundled manifest with mechanical flags and an A-SURV-03 human-review note — no curriculum rewrites.
