...
- Open `Survey CAD` from the toolbar to work in a standalone CAD drawing. Use the CAD file controls for `New Drawing`, `Open Drawing`, `Save Drawing`, `Export Drawing`, and `Import Adjusted Points`; Modify carries `Project Transform` (whole-drawing Helmert/Grid-Ground, see `docs/evidence/phase18r-project-transform-architecture.md`); adjustment projects no longer auto-own CAD state.
