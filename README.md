...
- Parser diagnostics: apostrophe-tagged station descriptions on `C/P/PH/CH/EH/E` records are scanned and grouped by canonical station ID; `.DESC`/`/DESC` now controls reconciliation policy (`FIRST` or `APPEND` with custom delimiter), parse state/logs report repeated/conflicting IDs with line traceability, and reconciled descriptions are applied in report/export/listing coordinate-style outputs.
