...
- Auto-sideshot detection (Phases 1-3): Project Options -> Adjustment now includes an `Auto-Sideshot` toggle (default ON) and parser command parity via `.AUTOSIDESHOT`/`/AUTOSIDESHOT`; when enabled, solver computes redundancy-aware candidate detection for non-redundant `M` records (with fixed-control target safety exclusion), emits run-log diagnostics, surfaces candidate tables in report/summary/export/listing outputs, and annotates adjusted-observation rows with auto-sideshot markers (`AUTO-SS` / `[auto-ss]`).
