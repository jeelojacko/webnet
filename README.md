...
- Auto-sideshot detection (Phases 1-2): Project Options -> Adjustment now includes an `Auto-Sideshot` toggle (default ON) and parser command parity via `.AUTOSIDESHOT`/`/AUTOSIDESHOT`; when enabled, solver computes redundancy-aware candidate detection for non-redundant `M` records (with fixed-control target safety exclusion), emits run-log diagnostics, and surfaces candidate tables in Adjustment Report, Processing Summary, WebNet export text, and industry-style listing output.
