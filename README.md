...
- Cluster detection (Phase 1): post-adjust candidate detection is now available with deterministic cluster keys, selectable linkage mode (`single`/`complete`), and 2D/3D tolerances; candidates are shown in report, processing summary, and both export formats.
...
- UX: `Project Options -> Listing File` now drives industry-style output composition (section toggles), sorting (coordinates and adjusted-observation rows), azimuth/bearing visibility, and adjusted-observation row cap.
...
- Parity planning: TODO now includes an Industry Standard v6-v14 gap list prioritized by workflow impact (Conventional first, then GPS, then Leveling).
- Parity planning: every item in the Industry Standard v6-v14 gap list now includes phased implementation steps for execution sequencing and milestone tracking.
