...
- Auto-adjust workflow (Phase 1): Project Options -> Adjustment now includes an `Auto-Adjust` toggle (default OFF, next to Cluster Detection) plus controls for `|t|` threshold, max cycles, and max removals/cycle; when enabled, solve runs iterative exclusion cycles with detailed cycle/removal logs.
