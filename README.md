...
- Parser: Star*Net-style inline options (.UNITS/.COORD/.ORDER/.2D/.3D/.DELTA/.MAPMODE/.LWEIGHT/.NORMALIZE/.END/.LONSIGN) plus C/E/CH/PH/EH and D/A/G/L records with fixity/std errs, HI/HT capture, and ft<->m normalization (P/PH lat/long projected to local EN using first P as origin; .LWEIGHT applied as fallback for leveling with ft lengths converted to km; DV/M delta-mode emit distance+dH; B bearing placeholder). Header toggles expose delta/map/normalize and .LWEIGHT defaults; see TODO.md for remaining codes to support.
