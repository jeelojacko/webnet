...
- Open `Survey CAD` from the toolbar to review the CAD roadmap plus the live native-model/renderer spike preview, first command-history actions (`Select All`, `Clear Selection`, `ERASE`, `Undo`, `Redo`), snap feedback (`point-node`, `endpoint`, `midpoint`, `nearest`), and adapter export seam.
