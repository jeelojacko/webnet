...
- Open `Survey CAD` from the toolbar to review the CAD roadmap plus the live native-model/renderer spike preview, first command-history actions (`Select All`, `Clear Selection`, `ERASE`, `Undo`, `Redo`), snap feedback (`point-node`, `endpoint`, `midpoint`, `intersection`, `nearest`), typed command input (`POINT`, `COGO PT`, `LINE`, `PLINE`, `INVERSE`, `MOVE`, `COPY`, `INTX`) with coordinate / azimuth-distance / survey bearing-distance entry, and adapter export seam.
